"""B-ACQ-3 — land five orthogonal FRED categories as CURRENT-value offline-current
frozen snapshots via the generic offline-current binder. Exact proven shape of
B-ACQ-4 (research/bacq4_land.py): adapter fred_json_api, enabled:false +
archival:true, bound to the already-registered fred_current_provider family (no
new family, no owner decision). Section 3.1 each series its own id, never aliased.
Section 22.4 acquisition only — no channel, no member, no weight. Owner ruling
2026-08-08: current_revised, BARRED from as-of/real-time claim. Zero network:
cache bytes ARE the source object (verified against manifest sha).
"""
import argparse
import glob
import hashlib
...
AS_OF = os.environ["BACQ3_ASOF"]
ATT = os.environ["BACQ3_ATT"]
...
def _manifest_notes():
    ...
    for line in open(CACHE / "manifest.jsonl"):
        r = json.loads(line)
        if r.get("status") == "ok" and r.get("fred_id"):
            notes[r["fred_id"]] = {
                "fetch_utc": r["fetch_utc"], "category": r["category"],
                "candidate": r.get("candidate"), "manifest_sha": r["sha256"]}
    return notes
...
def source_for(sid, cat):
    ...
    return {
        ...
        "label": (
            ...
            "snapshot; B-ACQ-3 orthogonal category %s; current_revised, BARRED "
            "from as-of / real-time claim per owner ruling 2026-08-08; NOT "
            "admitted to any channel per section 22.4]" % (title, sa, cat)),
        "max_bytes": 33554432,
...
def cache_for(sid, note):
    ...
    if sha != note["manifest_sha"]:
        raise SystemExit("SHA MISMATCH %s: %s vs %s"
                         % (sid, sha, note["manifest_sha"]))
    manifest = {
        "fetch_utc": note["fetch_utc"], "source_bytes_length": len(body),
        "source_sha256": sha,
...
def main():
    ...
    for sid in sids:
        note = notes[sid]
        src = source_for(sid, note["category"])
        ...
        if a.dry:
            ...
            plan.append({"series_id": sid, "source_id": src["source_id"],
                         "category": note["category"], "records": len(recs),
    ...
    if a.dry:
        json.dump(plan, open("research/BACQ3_plan.json", "w"), indent=1)
        print("DRY: %d series gated+parsed, no writes" % len(plan))
        for p in plan:
            print(" ", p["category"], p["series_id"], p["records"],
                  p["obs_min"], "->", p["obs_max"])
        return
    ...
    json.dump({"bound": bound, "pointer": ptr},
              open("research/BACQ3_land_out.json", "w"), indent=1)
