"""B-ACQ-LEI-CEI — land the free Conference Board LEI/CEI benchmark components as
CURRENT-value offline-current frozen snapshots via the generic offline-current
binder. Exact shape of B-OFFLINE-2 / B-ACQ-5 (research/bacq5_land.py): adapter
fred_json_api, enabled:false + archival:true, bound to the already-registered
fred_current_provider family (no new family, no owner decision). section 3.1
each series lands under its own id; S&P 500 is NOT NASDAQ; consumer expectations
proxy UMCSENT lands under its own id. section 22.4 acquisition only (landing
PAYEMS does NOT admit it to a channel). Owner ruling 2026-08-08: current_revised,
BARRED from as-of/real-time claim. Zero network: cache bytes ARE the source
object. Clobber-proof: build_snapshot walk filters already-present series_ids.
"""
import argparse
import glob
import hashlib
...
AS_OF = os.environ["BACQLC_ASOF"]
ATT = os.environ["BACQLC_ATT"]
...
def _freq_word(short):
    return {"Daily": "daily", "Daily, Close": "daily", "Weekly": "weekly",
            "Monthly": "monthly", "Quarterly": "quarterly",
            "Annual": "annual"}.get(short, short.lower())
...
def source_for(sid, note):
    ...
    return {
        ...
        "label": (
            ...
            "snapshot; B-ACQ-LEI-CEI Conference Board LEI/CEI benchmark component; "
...
def main():
    ...
    if a.dry:
        json.dump(plan, open("research/BACQLC_plan.json", "w"), indent=1)
    ...
    json.dump({"bound": bound, "pointer": ptr},
              open("research/BACQLC_land_out.json", "w"), indent=1)
