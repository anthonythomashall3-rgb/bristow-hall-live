"""B-ACQ-4 — land Empire (NY) + Dallas regional Fed survey diffusion indices as
...
district its own series id, never aliased onto PHILLY. §22.4 acquisition only.
...
AS_OF = os.environ["BACQ4_ASOF"]
ATT = os.environ["BACQ4_ATT"]
...
def source_for(sid, note):
    ...
    return {
        ...
        "label": (
            ...
            "snapshot; B-ACQ-4 regional Fed survey diffusion index; "
...
def main():
    ...
    if a.dry:
        json.dump(plan, open("research/BACQ4_plan.json", "w"), indent=1)
    ...
    json.dump({"bound": bound, "pointer": ptr},
              open("research/BACQ4_land_out.json", "w"), indent=1)
