"""B-ACQ-NFCI-COMPONENTS — land the free, exact-identity FRED NFCI *input material*
as CURRENT-value offline-current frozen snapshots. Same proven shape as B-ACQ-5 /
B-ACQ-LEI-CEI (adapter fred_json_api, enabled:false + archival:true, bound to the
already-registered fred_current_provider family; ZERO new parser shapes, §6.2).
§3.1 each series its own id. §22.4 ACQUISITION ONLY — sets no weight, creates no
channel, does not build finconditions. Owner ruling 2026-08-08: current_revised,
BARRED from as-of/real-time claim; the bar is recorded in each label.

Land set (clean free-FRED components not already in store, not reserved to
B-CERT-MARKET / B-CERT-MARKET-R2):
  OVXCLS       exact identity of NFCI risk indicator #16 (CBOE Crude Oil Vol OVX)
  MORTGAGE30US raw conforming-mortgage-rate ingredient of NFCI credit mortgage-spread
               indicators #4 (jumbo) and #8 (conforming); flagged raw-material, NOT
               the CFED-computed spread (§3.1 distinct identity).
Zero network in this step: cache bytes ARE the source object.
"""
import argparse
import hashlib
import json
...
CACHE = ROOT / "research/prefetch/bacqnfci"
AS_OF = os.environ["BACQNFCI_ASOF"]
ATT = os.environ["BACQNFCI_ATT"]

# per-id NFCI role note kept verbatim in the label
ROLE = {
    "OVXCLS": ("NFCI risk indicator #16 CBOE Crude Oil Volatility Index OVX "
               "(exact identity)"),
    "MORTGAGE30US": ("raw conforming-mortgage-rate ingredient of NFCI credit "
                     "mortgage-spread indicators #4/#8; NOT the CFED-computed spread"),
}
...
def source_for(sid):
    ...
    return {
        ...
        "label": (
            ...
            "snapshot; B-ACQ-NFCI-COMPONENTS %s; current_revised, BARRED from "
            "as-of / real-time claim per owner ruling 2026-08-08; NOT admitted to "
            "any channel and sets no finconditions weight per section 22.4]"
            % (title, sa, ROLE[sid])),
        "max_bytes": 33554432,
...
def main():
    ...
    if a.dry:
        json.dump(plan, open("research/BACQNFCI_plan.json", "w"), indent=1)
    ...
    json.dump({"bound": bound, "pointer": ptr},
              open("research/BACQNFCI_land_out.json", "w"), indent=1)
