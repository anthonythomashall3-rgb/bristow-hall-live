"""WALK 86 - THE VETTED LEGS (16 September 2026).

Pre-registered in PREREG-v347-2026-09-16.md before this was run.

WHY THIS WALK EXISTS. The frozen channel screen that chose legs G, J, F, A, H, D and O had never been
measured against a null. It has now been, three ways, and the result changed which legs deserve to be
in the system.

  1. POOLED NULL. Fake chronologies that relocate every NBER peak independently, drawn from months at
     least a year clear of every real recession. The true chronology draws 4,907 in-window
     configurations from a 1,500-channel sample; six fake ones draw 11,434, 7,874, 1,526, 3,431, 2,224
     and 1,266. The true chronology is not even the largest. The null is bimodal: fake peaks in calm
     stretches draw a mean of 16.5, and the fourteen highest-drawing fake peaks are ALL real episodes
     of stress that did not become recessions - the 1966-68 credit crunch, 2003, 2010-11, 2014, 2015,
     2021-23. The screen measures turbulence, not recession onset.

  2. PER-CHANNEL FALSE-DISCOVERY TEST. Re-run one channel alone against forty fake chronologies and
     compare how many admissible configurations it produces on the true chronology against the fake
     mean. On the standing [-92,-1] window the existing legs come out: BORROW (leg G) 30x, P=0.000;
     AUTHNOTT (leg H) 3.6x, P=0.075; BBKMCOIX (leg A) 3.4x, P=0.150; M1SL (leg J) 2.6x, P=0.200; and
     WBUSAPPWNSAUS (leg F) 0.04x, P=0.525 - it clears a WRONG chronology more easily than the right
     one. Walk 85's three fitted legs fail too, which is consistent with walk 85 having refused them.

  3. SIBLING TEST. D3WACL - coin holdings at the Federal Reserve Bank of Philadelphia - produced 548
     admissible configurations against a fake mean of 0.1, a ratio of eleven thousand to one, the most
     significant channel in 5,761. Running the other eleven districts takes a minute: seven produce
     nothing. Statistical significance is not a mechanism, and the sibling test is what separates them.

WHAT SURVIVES, AND IS OFFERED HERE. One family, arrived at from 5,761 channels without knowing which
were already in use: central bank and commercial bank balance sheets. On the same test, with P=0.000
throughout - commercial bank cash assets 680x, Federal Reserve total assets 337x, primary credit 126x,
total borrowings 118x, industrial production of durable consumer goods 111x, secondary credit 38x.
Leg G is in that family and is kept. Legs A, J and F are not and are dropped.

  leg E  TOTRA, Federal Reserve total resources and assets, weekly from 1914.
  leg S  CASACBW027NBOG, cash assets of all commercial banks. Its H.8 family passes the sibling test
         11 of 14, covering 1973, 1981, 1990, 2001 and 2007.
  leg T  IPDCONGD, industrial production of durable consumer goods, monthly from 1947.
  leg Z  TOTBORR, total borrowings of depository institutions from the Federal Reserve.

Letters E, S, T and Z were checked against the core's (B C I K L U V W X) and against every amendment
letter used so far (A M N H D O Y G J F P Q R) BEFORE pricing - the leg-letter collision that voided
walk 73 is not repeated.

The legs enter as ONE LADDER declared before the run: None, Y, YG, YGE, YGES, YGEST, YGESTZ.

TWO THINGS THIS WALK CANNOT SETTLE. The screen that found E, S, T and Z is frozen - its settings were
chosen with the whole sample visible - so if this walk refuses them it is the sixth pre-registered
refusal of pre-screened legs and will be reported as such. And TOTRA ends 2018-04-11 and has exactly
one ALFRED vintage, so leg E is historical evidence for a mechanism, not a forward-looking channel;
carrying it forward needs a splice to WALCL that is justified on overlap first.

Run:  python3 walk86.py 1962 2026 w86"""
...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk86_%s.out")
...
GRID = GRID + [('warnw', [26, 13, None]), ('warnp', [95, 90, 85]), ('newlegs', [None, 'Y', 'YG', 'YGE', 'YGES', 'YGEST', 'YGESTZ'])]
