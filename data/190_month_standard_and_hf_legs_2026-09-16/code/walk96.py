"""WALK 96 - WALK 94 WITH THE HIGH-FREQUENCY TRANSMISSION LEGS ON THEIR OWN LADDER AXIS (16 September 2026).
Pre-registered in 190_month_standard_and_hf_legs_2026-09-16/docs/PREREG-v356-walk96-2026-09-16.md before this was run.
The legs (payments, physical activity, plumbing, hiring, credit cost; daily or weekly, never revised) are the survivors
of the span-matched null in collection 190 (hf_legs.select), built exactly as the screen built them, admitted through
the same 400-day arming bar as legs A..m, on a grid axis `hf` of their own: None, each leg alone, all together.
Everything else is walk 94's. Run:  python3 walk96.py 1962 2026 w96"""
...
# ---- THE HIGH-FREQUENCY LEGS (collection 190): selected by hf_legs.select from the span-matched null, built by the
# ---- screen's own engine; nothing re-tuned here. Letters are symbols no other leg uses.
sys.path.insert(0, os.path.join(_ROOT, '190_month_standard_and_hf_legs_2026-09-16', 'code'))
import hf_legs as _HF
_HFSEL = pd.read_csv(os.path.join(_ROOT, '190_month_standard_and_hf_legs_2026-09-16', 'out', 'hf_legs_selected.csv'))
_HFLEGS = _HF.build(_HFSEL)
assert not (set(_HFLEGS) & (_CORE_LETTERS | _USED_LETTERS | _NEW86)), 'hf leg letter collision'
NEW_LEGS.update(_HFLEGS)
HF_ALL = ''.join(_HFSEL['letter'].tolist())
print('hf legs:', {k: len(v) for k, v in _HFLEGS.items()}, flush=True)


_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk96_%s.out")
...
_new = _old + """
    ...
    for _L in ((p.get('newlegs') or '') + (p.get('extra') or '') + (p.get('hf') or '')):
...
GRID = GRID + [('warnw', [26, 13, None]), ('warnp', [95, 90, 85]), ('newlegs', [None, 'A', 'AY', 'AYG', 'AYGJ', 'AYGJF']), ('extra', [None, 'c', 'm', 'cm']), ('hf', [None] + list(HF_ALL) + [HF_ALL])]
NAMES = [n for n, _ in GRID]; GD = dict(GRID)
BASE15 = dict(BASE15); BASE15.update(warnw=None, warnp=90, newlegs=None, extra=None, hf=None)
