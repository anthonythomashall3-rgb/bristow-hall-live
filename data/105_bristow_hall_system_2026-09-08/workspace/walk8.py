"""THE WALK-FORWARD DIARY WITH THE SURVEY-WEEK OBJECT. The eighth object is the insured unemployment rate in the week
...
import sys,pickle
exec(open('walk3.py').read().split('CACHE6={}')[0].replace("out=open('walk3_%s.out'%sys.argv[1],'w')","out=open('walk8_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')"))
...
def ev9(p):
    ...
    return CACHE9[k]
BASE9=dict(BASE); BASE9['deep']=15; BASE9['wline']=None; BASE9['wline2']=None
...
GRID=[('sahm',[0.50,0.45,0.43,0.40]),('vl',[0.30,0.25,0.20,0.15]),('u45',[0.55,0.45,0.35]),('low',[0.30,0.25,0.20]),
      ...
      ('wline',[None,0.40,0.35,0.30,0.25]),('wline2',[None,0.60,0.50,0.45])]
...
if 'keep' in VAR:
    ...
    GRID=[(n,([x for x in g if x is not None]+[None]) if n in ('wline','wline2') else g) for n,g in GRID]
def clean(q,cut,ks):
    r,t=ev9(q)
