...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr3.out','w')"))
def build_v(p):
...
c=bristow_closer('the insured rate, inverted',0.02,3,12,3)
detail('with the Bristow rule added, one confirmation',{**TLH,'S':S2,'R':confirm_close2(c,0.10,2.0,1)})
detail('with the Bristow rule added, two confirmations',{**TLH,'S':S2,'R':confirm_close2(c,0.10,2.0,2)})
out.close()
