...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr.out','w')"))
def build_v(p):
...
def bristow_closer(nm,band,n,L,stable,lead=9):
    ...
    return res
P("THE BRISTOW RULE ITSELF AS THE CLOSER, run in real time on four levels")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  the shipped closers with S rebuilt',{**TLH,'S':S2})
import itertools
for nm in LVL:
    for band,n,L,stable in itertools.product([0.02,0.05],[3],[12],[1,2,3]):
        c=bristow_closer(nm,band,n,L,stable)
        if not c: continue
        trow(f'  Bristow on {nm}, band {band}, settles in {stable}m ({len(c)} calls)',{'R':c,'S':S2})
        trow(f'  the same beside the shipped closers',{**TLH,'S':S2,'R':c})
out.close()
