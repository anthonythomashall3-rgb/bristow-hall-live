...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk79_%s.out")
...
def obj(vw):
    # WALK 79's ONE CHANGE, and it is a tie-break, not a new counting key.
    # The first key is walk 78's -- a call on or after the peak, or more than three months early, is a
    # miss -- and it is untouched. What changes is what decides BETWEEN configurations that miss equally
    # often. v3.29's keys were the median and mean of the SIGNED lead, which a more negative number
    # makes smaller, so the walk has always preferred earlier calls without limit. The requirement does
    # not: a call anywhere in [-92, -1] satisfies it, and being 90 days early is not better than being
    # 5 days early. So among configurations with the same miss count, this prefers the one whose calls
    # sit closest to one day early.
    # THE WARNING, recorded before the run: walks 56, 58 and 64 each put a DISTANCE key into the
    # objective and each damaged the record. This is a distance key. It differs from those three in that
    # it is subordinate to the requirement's own counting key rather than added alongside v3.29's, but
    # that is an argument, not evidence, and the refutation conditions decide.
    v, w = vw
    a = (sum(1 for x in v if x > -1 or x < -92),
         float(np.median([abs(x + 1) for x in v])), float(np.mean([abs(x + 1) for x in v])))
