"""WALK 89 - THE SAME LADDER; TIE-BREAKER = SAFETY COUNT, THEN TWO-MONTH TARGET (16 September 2026).

Pre-registered in PREREG-v350-2026-09-16.md before this was run.
...
Run:  python3 walk89.py 1962 2026 w89"""
...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk89_%s.out")
...
def obj(vw):
    ...
    # WALK 89. Walk 88 aimed the tie-breaker at thirty days early and lost 2007 to a four-day-late call:
    # the earliness preference it removed was the safety margin. So the second key here is a SAFETY
    # count -- calls inside fifteen days of the peak, which one revision could turn late -- added to
    # the count of calls more than two months early (the user's target). Only after both does the walk
    # aim the median at forty days early. The first key is unchanged.
    TARGET = -40.0
    a = (sum(1 for x in v if x > -1 or x < -92),
         sum(1 for x in v if x < -61) + sum(1 for x in v if -15 <= x <= -1),
         abs(float(np.median(v)) - TARGET), abs(float(np.mean(v)) - TARGET))
