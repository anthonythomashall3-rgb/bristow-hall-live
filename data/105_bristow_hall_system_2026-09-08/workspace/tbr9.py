...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr9.out','w')"))
def build_v(p):
...
P("WHAT THE SETTLING TEST ITSELF OFFERS AT EACH TROUGH, before and after confirmation, against what was taken")
for back,stable,need in [(48,3,2),(36,3,1),(24,2,1),(18,2,1)]:
    c=bristow_closer('the insured rate, inverted',0.02,3,12,stable,back=back)
    cc=confirm_close2(c,0.10,2.0,need)
    P(f"\n   window {back}m, settles {stable}m, {need} confirmation")
    for i in range(13):
        tr=TR[i]
        raw=[(a,b) for a,b in c if tr-pd.DateOffset(months=6)<=b<=tr+pd.DateOffset(months=12)]
        con=[(a,b) for a,b in cc if tr-pd.DateOffset(months=6)<=b<=tr+pd.DateOffset(months=12)]
        r0=f"{raw[0][0]:%Y-%m-%d}/{raw[0][1]:%Y-%m} ({(raw[0][0]-me(tr)).days:+d})" if raw else "-"
        c0=f"{con[0][0]:%Y-%m-%d}/{con[0][1]:%Y-%m} ({(con[0][0]-me(tr)).days:+d})" if con else "-"
        P(f"      {tr:%Y-%m}  its own call {r0:34s} after confirmation {c0}")
out.close()
