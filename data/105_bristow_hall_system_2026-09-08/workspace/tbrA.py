...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbrA.out','w')"))
def build_v(p):
...
def confirm_claims_only(calls,cdrop,fwd=6):
    """the fastest confirmation there is: the claims four-week mean off its own peak, published five days after the
    week ends. Before the claims series exists a call passes through unconfirmed."""
    out_=[]
    for pp,dd in calls:
        if dd<BELOWc.index.min(): out_.append((pp,dd)); continue
        seg=BELOWc[(BELOWc.index>=dd)&(BELOWc.index<=dd+pd.DateOffset(months=fwd))]
        h=seg[seg>=cdrop]
        if len(h): out_.append((max(pp,h.index[0]+pd.Timedelta(days=5)),dd))
    return out_
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
P("THE CONFIRMATION IS WHAT IS SLOW. The settling test offers 2001 at +12 and 2020 at +12; the confirmation pushes")
P("them to +108 and +66. The fastest confirmation available is the claims flow itself, weekly, five days after the week.")
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
import itertools
for back,stable,cd in itertools.product([24,36,48],[2,3],[0.05,0.10,0.15,0.20]):
    c=bristow_closer('the insured rate, inverted',0.02,3,12,stable,back=back)
    if not c: continue
    cc=confirm_claims_only(c,cd)
    if not cc: continue
    trow(f'  settling {back}m/{stable}m, claims-only confirmation at {cd}',{**TLH,'S':S2,'R':cc})
P("")
P("and why 2024 has no settling call: what each level was doing")
for nm in LVL:
    lv=LVL[nm]; w=lv[(lv.index>=pd.Timestamp('2024-01-01'))&(lv.index<=pd.Timestamp('2025-06-01'))]
    P(f"   {nm}: min at {w.idxmin():%Y-%m}, last {w.index[-1]:%Y-%m}, still at its low in the last month: {w.idxmin()==w.index[-1]}")
out.close()
