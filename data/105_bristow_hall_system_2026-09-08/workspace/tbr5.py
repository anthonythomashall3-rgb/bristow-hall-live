...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr5.out','w')"))
def build_v(p):
...
def bristow_weekly(band=0.02,n=4,L=52,stable=3,back_w=156,pub=5):
    ...
    return out_
P("EVERY OBJECT GIVEN THE SETTLING TEST, each confirmed, the chronology taking the earliest that is clean")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
CS={}
for nm in LVL:
    for stable in [2,3,4]:
        c=bristow_closer(nm,0.02,3,12,stable)
        if c: CS[(nm,stable)]=c
P(f"   settling calls built: "+", ".join(f"{nm.split(',')[0]}/{st}m:{len(v)}" for (nm,st),v in CS.items()))
import itertools
best=None
for stable in [2,3,4]:
    for need in [1,2]:
        TL={**TLH,'S':S2}
        tag=[]
        for j,nm in enumerate(LVL):
            if (nm,stable) not in CS: continue
            cc=confirm_close2(CS[(nm,stable)],0.10,2.0,need)
            if cc: TL['R%d'%j]=cc; tag.append(nm.split(',')[0])
        if len(TL)<=5: continue
        trow(f'  all settling tests, {stable}m settle, {need} confirmation ({len(tag)} objects)',TL)
P("")
P("and the insured rate alone at each settling length, one confirmation")
for stable in [2,3,4]:
    if ('the insured rate, inverted',stable) in CS:
        trow(f'  insured rate, settles {stable}m, 1 confirmation',{**TLH,'S':S2,'R':confirm_close2(CS[('the insured rate, inverted',stable)],0.10,2.0,1)})
out.close()
