...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pS.out','w')"))
def build_v(p):
...
def full(nm,p,TL):
    ...
    P(f"     onsets {lp}")
    return turns
P("does the tool still work without the S closer, which is Paper 1's rule and which closed February 1970 inside the new fast 1969 episode?")
full('all four closers K J H S',BASE39,TLH)
for drop in ['S','J','K','H']:
    TL={k:v for k,v in TLH.items() if k!=drop}
    full(f'without {drop}',BASE39,TL)
out.close()
