...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pR.out','w')"))
def build_v(p):
...
def confirm_close(calls,cdrop,rise,pubday_starts=18,pubday_hours=5,fwd=6,need=1):
    ...
    return out_
P("the 8 January 1981 call, dated December 1980, is the only thing left. What was above its line that month?")
p=dict(BASE39)
G=vgap2(p['vk'],p['vb']); Hc,MX=mkpair3(p['starts'],p['half'],3,p['minw']); Hc=dict(Hc); Hc['line']=p['hline']; Hh=mkhours(p['hrs'],p['nd'])
for m in pd.date_range('1980-06-01','1981-06-01',freq='MS'):
    sp=GSP[(GSP.index>=m)&(GSP.index<m+pd.DateOffset(months=1))]
    n=sum([G.get(m,-9)>=p['vl'], Hc['gap'].get(m,-9)>=Hc['line'], (sp.max() if len(sp) else -9)>=p['spr'], Hh['gap'].get(m,-9)>=1.0])
    P(f"   {m:%Y-%m}  vacancy {G.get(m,float('nan')):6.3f}/{p['vl']}  housing {Hc['gap'].get(m,float('nan')):6.3f}/{Hc['line']}  spread {(sp.max() if len(sp) else float('nan')):6.3f}/{p['spr']}  hours {Hh['gap'].get(m,float('nan')):4.1f}  -> {n} above their lines")
P("")
P("and for comparison, the months the other fast calls were dated")
for m in [pd.Timestamp('1969-07-01'),pd.Timestamp('1973-11-01'),pd.Timestamp('1979-11-01'),pd.Timestamp('2000-12-01'),pd.Timestamp('2007-11-01'),pd.Timestamp('2024-01-01')]:
    sp=GSP[(GSP.index>=m)&(GSP.index<m+pd.DateOffset(months=1))]
    n=sum([G.get(m,-9)>=p['vl'], Hc['gap'].get(m,-9)>=Hc['line'], (sp.max() if len(sp) else -9)>=p['spr'], Hh['gap'].get(m,-9)>=1.0])
    P(f"   {m:%Y-%m}  vacancy {G.get(m,float('nan')):6.3f}  housing {Hc['gap'].get(m,float('nan')):6.3f}  spread {(sp.max() if len(sp) else float('nan')):6.3f}  hours {Hh['gap'].get(m,float('nan')):4.1f}  -> {n} above their lines")
out.close()
