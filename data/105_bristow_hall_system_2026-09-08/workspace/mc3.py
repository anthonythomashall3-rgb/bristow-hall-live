...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc3.out','w')"))
def build_v(p):
...
def multichannel_closer(band=0.02,n=3,L=12,stable=3,back=48,q=0.5,minch=3):
    ...
    return out_
P("2024: WHICH CHANNELS SETTLE AT ALL, AND WHEN. The trough is August 2024 and S closes it on 18 January 2025 (+140).")
SM={nm:(lv.rolling(3).mean().dropna(),d) for nm,(lv,d) in CH.items()}
for nm,(lv,d) in SM.items():
    got=[]
    for m in pd.date_range('2024-08-01','2026-06-01',freq='MS'):
        avail=lv[lv.index<=m-pd.DateOffset(months=1)]
        if not len(avail): continue
        dt=ch_date(avail,m-pd.DateOffset(months=48),avail.index[-1],0.02,1,12)
        if dt is not None and pd.Timestamp('2024-01-01')<=dt<=pd.Timestamp('2025-06-01'):
            got.append((m,dt))
    if got:
        first=got[0]
        P(f"   {nm:20s} first settles at {first[1]:%Y-%m}, seen from evaluation month {first[0]:%Y-%m}, published {_pub(nm,first[0]-pd.DateOffset(months=1)):%Y-%m-%d} ({(_pub(nm,first[0]-pd.DateOffset(months=1))-me(TR[12])).days:+d} days)")
    else:
        P(f"   {nm:20s} never settles inside the 2024 window")
P("")
P("the levels themselves through 2024 and 2025, three-month means")
for nm,(lv,d) in SM.items():
    w=lv[(lv.index>=pd.Timestamp('2024-01-01'))&(lv.index<=pd.Timestamp('2025-06-01'))]
    if not len(w): continue
    P(f"   {nm:20s} min at {w.idxmin():%Y-%m}  value {w.min():.3f}  last {w.index[-1]:%Y-%m}  still at its low: {w.idxmin()==w.index[-1]}")
out.close()
