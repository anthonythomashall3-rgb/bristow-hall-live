...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc9.out','w')"))
def build_v(p):
...
ROLL={'unemployment rate':(UR.dropna(),(lambda m: rel.get(m,pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=4)))),
      'insured rate':(spl.dropna(),(lambda m: pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=10))),
      'initial claims':(_CLm,(lambda m: pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=4))),
      'continued claims':(_CCm,(lambda m: pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=4)))}
def rolled(nm,k=1,tol=0.0):
    lv,pf=ROLL[nm]; s3=lv.rolling(3).mean().dropna(); out_={}
    for m,v in s3.items():
        p_=m-pd.DateOffset(months=k)
        if p_ in s3.index and v<=s3[p_]-tol: out_[m]=pf(m)
    return out_
def confirm_rolled(calls,nm,k,fwd=6):
    RO=rolled(nm,k); out_=[]
    for pp,dd in calls:
        hits=[p_ for m,p_ in RO.items() if dd<=m<=dd+pd.DateOffset(months=fwd)]
        if hits: out_.append((max(pp,min(hits)),dd))
    return out_
P("THE SETTLING TEST ON EVERY DEMAND CHANNEL, GUARDED BY A LABOUR OBJECT THAT HAS STOPPED RISING.")
P("A demand object settling is fast but fires in expansions; a labour object that has stopped rising cannot fire")
P("before the labour market has turned. One of each, on their own clocks.")
...
import itertools
for ch,gu,stable,k in itertools.product(['housing starts','weekly hours','vacancy rate','paper spread'],
                                        ['unemployment rate','insured rate','initial claims','continued claims'],[4,6],[1,2]):
    c=first_to_settle(stable=stable,back=48,chans=[ch])
    if not c: continue
    cc=confirm_rolled(c,gu,k)
    if not cc: continue
    trow(f'  {ch} settling {stable}m, guarded by {gu} rolled over {k}m',{**base,'R':R1,'T':cc})
out.close()
