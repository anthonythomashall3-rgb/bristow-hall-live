...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tnine2.out','w')"))
...
ALL=sorted(RES,key=lambda r:(-r['w31b'],-r['w31'],r['med']))
P("the twenty fastest by within-the-month, clean or not, with the reason each fails")
P(f"{'w31/13':>7s} {'from1970':>9s} {'closes':>7s} {'prem':>5s} {'othcl':>6s} {'onsetFA':>8s} {'exact':>6s}  configuration")
for r in ALL[:20]:
    P(f"{r['w31']:>7d} {r['w31b']:>9d} {r['n']:>7d} {r['prem']:>5d} {r['oth']:>6d} {r['fa']:>8d} {r['ex']:>6d}  {r['tag']}")
BESTF=[r for r in ALL if r['n']==13][:6]
for r in BESTF:
    P(f"\n   {r['tag']}  (prem {r['prem']}, outside {r['oth']}, onset false alarms {r['fa']})")
