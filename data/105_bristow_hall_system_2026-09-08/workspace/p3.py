...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('p3.out','w')"))
def build_h(p):
...
import itertools
for hl,sp_,vl_ in itertools.product([1.0,0.95,0.90],[1.6,round(LINE,3),1.1,0.9],[0.20,0.15,0.12]):
