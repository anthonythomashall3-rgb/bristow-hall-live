...
# ---- THE HEIGHT INSIDE A RECESSION IS ITS DAMAGE (Anthony, 17 September 2026): one line, two meanings. Outside an
# ---- episode the line is the rule's reading, below 1.00 (the distance to the line). From the day the rule opened a
# ---- recession to the day it closed it, the line reads 1.00 plus the recession's damage so far: the unemployment rate's
# ---- latest first print, as published by that day, less its lowest first print of the twelve months known on the call
# ---- day. Real time: a month's print enters on its release day; nothing is revised in. 1.00 still means "the rule fired".
...
def _damage(d,call):
    known=_ufp[_ufp['published']<=d]
    if known.empty: return 0.0
    base=_ufp[_ufp['published']<=call].tail(12)['value']
    if base.empty: return 0.0
    return float(max(0.0,float(known.iloc[-1]['value'])-float(base.min())))
DMG=[]
for i,d in enumerate(DD):
    if DP[i]=='open' and d>=SITE_START:
        call=max([a for a,b in _EPD if a<=d<=b],default=None)
        if call is not None: dmg=_damage(d,call); DMG.append(round(dmg,2)); DV[i]=1.0+dmg; continue
    DMG.append(None)
