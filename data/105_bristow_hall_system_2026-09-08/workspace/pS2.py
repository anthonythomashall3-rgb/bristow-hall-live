...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pS2.out','w')"))
def build_v(p):
...
def confirm_close(calls,cdrop,rise,pubday_starts=18,pubday_hours=5,fwd=6,need=1):
    ...
    return out_
P("what the claims flow and the demand objects were doing at the two months S fires inside an episode")
for dd in [pd.Timestamp('1969-10-01'),pd.Timestamp('2024-08-01')]:
    b=BELOW[(BELOW.index>=dd)&(BELOW.index<=dd+pd.DateOffset(months=6))]
    P(f"   S dated {dd:%Y-%m}: claims below their own 52-week maximum over the next six months, max {b.max():.3f} (needs the line); starts above their low {RS[(RS.index>=dd)&(RS.index<=dd+pd.DateOffset(months=6))].max():.1f}%; hours {RH[(RH.index>=dd)&(RH.index<=dd+pd.DateOffset(months=6))].max():.1f}%")
P("")
P("S REBUILT: it proposes the close, a second object confirms")
full('  S as inherited',BASE39,TLH)
import itertools
for cdrop,rise,need in itertools.product([0.05,0.10,0.15,0.20],[1.0,2.0,3.0,5.0],[1,2]):
    TL=dict(TLH); TL['S']=confirm_close(TLG['S'],cdrop,rise,need=need)
    if not TL['S']: continue
    full(f'  S confirmed: claims {cdrop} off peak or a demand object +{rise}%, n={need}',BASE39,TL)
out.close()
