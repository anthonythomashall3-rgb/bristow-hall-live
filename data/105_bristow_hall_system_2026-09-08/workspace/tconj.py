...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tconj.out','w')"))
p0=dict(BASE)
...
def prop_survey(drop,pub=12):
    ...
    return out_
def confirm_t(calls,frac,back=6,fwd=4):
    """a close is confirmed when a demand object has come back BELOW its line inside the window on the dated month"""
    outc=[]
    for pp,dd in calls:
        best=None
        for cf in REC:
            ser=cf['gap']; seg=ser[(ser.index>=dd-pd.DateOffset(months=back))&(ser.index<=dd+pd.DateOffset(months=fwd))]
            hit=seg[seg<=cf['line']*frac]
...
def score(nm,TL):
    ...
    r=score13(turns)
    P(f"{nm:50s} closed {len(rows):2d}/13 median {np.median(lg):5.0f} mean {np.mean(lg):5.0f} worst {max(lg):4d} exact {sum(1 for e in er if e==0):2d} w1 {sum(1 for e in er if abs(e)<=1):2d} PREMATURE {prem} other {len(oth)} | onsets {len(r['lags_p'])}/13 falsealarm {len(r['other'])}")
    return rows
...
for d in [0.20,0.15,0.12,0.10,0.08,0.06]:
    for fr in [1.0,0.75,0.5]:
        ...
        score(f'  claims drop {d} confirmed at {fr:.2f} of the line',TL)
...
for d in [1.0,0.8,0.6,0.5,0.4]:
    for fr in [1.0,0.75,0.5]:
        ...
        score(f'  survey-week fall {d} confirmed at {fr:.2f} of the line',TL)
out.close()
