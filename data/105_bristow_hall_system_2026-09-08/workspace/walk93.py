"""WALK 93 - WALK 90 WITH A BOUNDED CONFIRMATION HOLD, WALKED (16 September 2026).
Pre-registered in PREREG-v353-walk93-2026-09-16.md before this was run.

A new grid parameter `hold` in {0, 15, 30, 45} days, first value 0. When a leg other than the core hub opens an
episode with a proposal published on day d, the call is published on the earlier of d + hold and the first
publication day after d, inside (d, d + hold], of any other proposer's surviving proposal. The dated month, the
closes and every leg's reading are unchanged: the hold delays publication and nothing else. Objective keys:
(1) calls outside [-92, -1]; (2) calls outside [-45, -1]; (3) median lead; (4) mean lead; trough keys unchanged.
Everything else is walk 90. Cache: the clean cache, every summary recomputed on 16 September.
Run:  python3 walk93.py 1962 2026 w93"""
...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk93_%s.out")
...
_HOLD_OLD = "    return score13(turns),turns"
_HOLD_NEW = """    _h = int(p.get('hold') or 0)
    if _h > 0:
        for _x in turns:
            if _x['kind'] != 'peak' or _x.get('leg') in ('X', 'hub'): continue
            _d = _x['published']; _cands = [_d + pd.Timedelta(days=_h)]
            for _L2, _lst in legs.items():
                if _L2 == _x.get('leg'): continue
                _cands += [a_ for a_, *_rest in _lst if _d < a_ <= _d + pd.Timedelta(days=_h)]
            _x['published'] = min(_cands)
    return score13(turns),turns"""
assert _hdr.count(_HOLD_OLD) == 1
_hdr = _hdr.replace(_HOLD_OLD, _HOLD_NEW)
exec(_hdr)
...
def obj(vw):
    v, w = vw
    a = (sum(1 for x in v if x > -1 or x < -92), sum(1 for x in v if x > -1 or x < -45), float(np.median(v)), float(np.mean(v)))
...
GRID = GRID + [('warnw', [26, 13, None]), ('warnp', [95, 90, 85]), ('newlegs', [None, 'A', 'AY', 'AYG', 'AYGJ', 'AYGJF', 'AYGJFc', 'AYGJFcm']), ('hold', [0, 15, 30, 45])]
NAMES = [n for n, _ in GRID]; GD = dict(GRID)
BASE15 = dict(BASE15); BASE15.update(warnw=None, warnp=90, newlegs=None, hold=0)
