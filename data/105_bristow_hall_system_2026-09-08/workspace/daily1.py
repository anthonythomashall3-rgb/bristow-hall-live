...
# THE WALK'S DAILY AND WEEKLY DATA ROOT. This was one hardcoded path into Recession Papers, and on 18 September
# 2026 EVERY WALK STOPPED WORKING because that folder had been moved: "Claude RA" now sits under Onset Detector
# Data, nested one level deeper than before. The failure was silent and far from its cause - L('NFCICREDIT')
# returned None and the chain died forty files later on "'NoneType' object has no attribute 'rolling'" - and it
# blocked every re-walk, which is to say every future version of the rule. The root is now looked for in each
# place it has lived, and the first one that actually holds the data wins. It is the same files either way; a
# moved folder must never again stop the rule from being rebuilt.
# Anthony confirmed on 18 September that the permanent home is Onset Detector Data/Claude RA/Claude RA, so that
# is tried first; the old Recession Papers path and the undoubled form are kept as fallbacks.
_DDC=[W.replace('24_bristow_rule_lab/workspace','Claude RA/Claude RA/paper2_exploration_2026-09-03/data_fetched'),
      W.replace('24_bristow_rule_lab/workspace','Claude RA/paper2_exploration_2026-09-03/data_fetched'),
      W.replace('24_bristow_rule_lab/workspace','../Recession Papers/Claude RA/paper2_exploration_2026-09-03/data_fetched')]
DD=next((c for c in _DDC if os.path.isdir(os.path.join(c,'extra')) or os.path.isdir(os.path.join(c,'fred_daily'))), _DDC[0])
if not os.path.isdir(DD):
    raise RuntimeError('the walk cannot find its daily data root; tried: %s' % _DDC)
print('daily data root:', DD, flush=True)
import glob
def load_series(nm):
