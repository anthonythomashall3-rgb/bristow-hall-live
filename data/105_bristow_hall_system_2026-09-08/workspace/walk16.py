...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk16_%s.out'%sys.argv[1],'w')"))
def build_v(p):
...
GRID=[('sahm',[0.50,0.45,0.43,0.40,0.3667]),('vl',[0.30,0.25,0.20,0.15]),('hline',[1.00,0.95,0.90,0.85]),
      ('spr',[1.6,round(LINE,3),1.1]),('hback',[6,5,4,3]),('u45',[0.55,0.45,0.35]),('low',[0.30,0.25,0.20]),
...
SUMF='cache/w15_sum.pkl'; SUM=pickle.load(open(SUMF,'rb')) if os.path.exists(SUMF) else {}
def summary(p):
...
LOG=[]; CHOSEN={}
for Y,M in [(y,m) for y in range(Y0,Y1+1) for m in range(1,13)]:
    ...
    for x in t:
        if x['kind'] in ('peak','trough') and cut<=x['published']<cut+pd.DateOffset(months=1):
