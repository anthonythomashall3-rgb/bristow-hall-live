"""WALK 95 - WALK 94 WITH THE VIOLATION WINDOW AT TWO MONTHS (16 September 2026).
Pre-registered in PREREG-v355-walk95-2026-09-16.md before this was run. Anthony's rule: no call more than two
months early, none late, no false alarm. The first objective key counts calls outside [-61, -1] instead of
[-92, -1]; every other key, the ladder axes (newlegs, extra), the grid, the arming bar and the clean cache are
walk 94's. Walks 82-85 and 89 tried narrower windows on older machinery and each produced a late call out of
sample; this is the same question asked of the current machinery on the clean cache.
Run:  python3 walk95.py 1962 2026 w95"""
...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk95_%s.out")
...
def obj(vw):
    v, w = vw
    a = (sum(1 for x in v if x > -1 or x < -61), float(np.median(v)), float(np.mean(v)))
