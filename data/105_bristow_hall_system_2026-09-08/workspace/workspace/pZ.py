...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pZ.out','w')"))
def build_v(p):
...
import itertools
P(f'   the confirming objects all exist from {CLmin:%Y-%m}; before that a close passes through unconfirmed')
for h,j,need in itertools.product([8.0,6.0,5.0,4.0,3.0],[5.0,4.0],[2,3]):
    TL={'K':TLH['K'],'S':S2,'H':confirm_close2(mkH(h),0.15,2.0,need),'J':confirm_close2(mkJ(j),0.15,2.0,need)}
