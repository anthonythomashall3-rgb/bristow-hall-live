...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk84_%s.out")
...
def obj(vw):
    v, w = vw
    # WALK 84: the band BETWEEN one and two months, [-61, -31]. This one excludes -2 as well as -88, so
    # unlike walk 83 it actively pushes the 2001 call away from the peak rather than merely tolerating
    # it there. That matters because 2001 has sat at -2 in twenty-three of twenty-seven walks -- but not
    # all: walk 68 called it at -47 off the core's own leg U at a different setting, and walks 69 and 75
    # called it at -29 off leg A. Both are inside this band, so a configuration reaching it exists.
    # The risk is the same one walk 82 realised: a band that pushes calls later pushes some past the
    # peak. Walk 82 did exactly that, twice.
