...
exec(open('causal28.py').read().split('MODE=sys.argv[2]')[0].replace("out=open('causal28_%s.out'%(sys.argv[2] if len(sys.argv)>2 else 'fast'),'w')","out=open('score_diary.out','w')"))
import pickle,glob
START=pd.Timestamp(STARTS)
LOG=[]
for f in sorted(glob.glob(f'cache/{PAT}_*.pkl')):
    if '_chosen_' in f or '_carry' in f or '_sum' in f: continue
...
for pub,kind,dt,leg in LOG:
    ...
    if lagt<0: badclose.append((pub.strftime('%Y-%m-%d'),dt.strftime('%Y-%m'),'published before the trough month ended'))
