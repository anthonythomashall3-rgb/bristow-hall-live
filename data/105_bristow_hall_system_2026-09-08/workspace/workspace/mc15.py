...
src=open('walk24.py').read().split('BASE15=dict(BASE)')[0].replace("out=open('walk24_%s.out'%sys.argv[1],'w')","out=open('mc15.out','w')")
exec(src)
...
def wrun(lv,stable,back_w,pub,band=0.02,n=4,L=52):
    """for every week, whether the weekly settling test has been settled for `stable` weeks running, and from when"""
    st={};run=None;runlen=0
    for t in lv.index:
        w=lv[(lv.index>t-pd.Timedelta(weeks=back_w))&(lv.index<=t)]
        ok=False
        if len(w)>=8:
            ...
            if len(d):
                ...
                if i!=w.index[-1]:
                    ...
                    ok=runlen>=stable
                else: run=None;runlen=0
            else: run=None;runlen=0
        else: run=None;runlen=0
        st[t+pd.Timedelta(days=pub)]=ok
    return pd.Series(st).sort_index()
def nleg(series,stable,back_w,pub):
    """THE MONTHLY DATE ON THE WEEKLY TRIGGER. The monthly insured rate says which month; the weekly series says that
    the fall has stopped. The close is made at the monthly publication that follows a settled weekly reading."""
    ARM=wrun(series,stable,back_w,pub); ai=ARM.index; out_=[];seen=set()
    for m in _MI.index:
        pdm=_MIP[m]
        k=int(ai.searchsorted(pdm,side='right'))-1
        if k<0 or not bool(ARM.iloc[k]): continue
        dd=bris_date(pdm)
        if dd is None or dd in seen: continue
        seen.add(dd); out_.append((pdm,dd))
    return _confirm_close(out_)
...
def report(nm,TL):
    ...
    un=[x for x in tro if not any(TRt[i]-pd.DateOffset(months=6)<=x['date']<=TRt[i]+pd.DateOffset(months=12) for i in range(13))]
...
for st in (6,8,10,13):
    for bw in (156,208):
        TL=dict(TLH); TL['N']=nleg(_IC4,st,bw,5)
        report(f"v3.18 plus N: monthly insured date on a weekly CLAIMS trigger stable={st} back={bw}w",TL)
for st in (6,8,10,13):
    for bw in (156,208):
        TL=dict(TLH); TL['N']=nleg(_IU4,st,bw,12)
        report(f"v3.18 plus N: monthly insured date on a weekly INSURED trigger stable={st} back={bw}w",TL)
out.close()
