...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pX.out','w')"))
def build_v(p):
...
P("with the hub window back at SIX months, which is what the causal chooser picks, the December 2025 call returns")
for nv in [1,2,3]:
    p=dict(BASE39); p['hback']=6
    full4(f'  hub window 6 months, vacancy holds {nv} month(s)',p,TLS,nv)
P("")
P("and at five months, for comparison")
for nv in [1,2]:
    p=dict(BASE39); p['hback']=5
    full4(f'  hub window 5 months, vacancy holds {nv} month(s)',p,TLS,nv)
out.close()
