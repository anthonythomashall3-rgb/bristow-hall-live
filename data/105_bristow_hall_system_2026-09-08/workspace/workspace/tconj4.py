...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tconj4.out','w')"))
...
P("\nweekly claims propose; a demand object RISES from its own low to confirm")
for d in [0.15,0.12,0.10,0.08]:
    for pct in [1.0,2.0,3.0,5.0]:
        for nmin in [1,2]:
            TL=dict(TLH); TL['Q']=[(a,b) for a,b,c in confirm_r(prop_weekly(d),pct,nmin)]
            score(f'  drop {d} rise {pct}% n={nmin}',TL)
out.close()
