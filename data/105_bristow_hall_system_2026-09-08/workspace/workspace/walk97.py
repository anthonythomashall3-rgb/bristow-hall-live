"""WALK 94 - WALK 81 WITH LEGS c AND m OFFERED ON THEIR OWN LADDER (16 September 2026).
Pre-registered in PREREG-v354-walk94-2026-09-16.md before this was run.

Walk 90 appended c and m to walk 81's nested ladder (None, A, AY, AYG, AYGJ, AYGJF, AYGJFc, AYGJFcm). At the 1972
cut leg m improved the known record (1969), and to take m the walk had to take the whole ladder - A, Y, G, J, F
came along in 1972, twenty to fifty years before the cuts at which walk 81 took them (2002, 2009, 2021), and then
made the 2001, 2007 and 2020 calls (Y -57, G -81, F -86). A leg priced on later peaks rode in on an unrelated
improvement. The nesting is the defect. Here the two are separate grid parameters:

  newlegs  : None, A, AY, AYG, AYGJ, AYGJF        (walk 81's ladder, unchanged)
  extra    : None, c, m, cm                       (the two legs of the frozen composite that call earlier)

Objective, grid, cache (clean) and everything else are walk 81's.
Run:  python3 walk94.py 1962 2026 w94"""
import sys, pickle, os, csv, json
...
_IDX = pd.date_range('1946-01-31', max(pd.Timestamp('2026-09-30'), pd.Timestamp.today().normalize() + pd.Timedelta(days=45)), freq='D')   # 17 Sep 2026: the daily index runs past today (was fixed at 2026-09-30, after which no carried leg could have fired); the states beyond the last datum are its ffill, so no proposal can appear after the data
...
# the mechanism of every leg, and every leg's proposals, for the narrow bar's second-mechanism test. Written from
# the live state file by code/write_leg_mechanisms.py before the walk and frozen for it.
_LEGMECH = json.load(open('cache/leg_mechanisms.json'))
_ALLPROP = dict(NEW_LEGS)
for _k, _v in WARN_PROPOSALS.items(): _ALLPROP.setdefault('N', []).extend(_v)
try:
    _st = json.load(open('out/bhs_state.json'))
    for _l in _st.get('carried_legs') or []:
        _ps = [(pd.Timestamp(x), pd.Timestamp(x)) for x in (_l.get('proposals') or '').split()]
        if _ps: _ALLPROP.setdefault(_l['letter'], []).extend(_ps)
except Exception as _e:
    print('narrow bar: could not read carried-leg proposals:', _e, flush=True)
print('narrow bar: %d legs with proposals, %d distinct mechanisms'
      % (len(_ALLPROP), len({_LEGMECH.get(k, k) for k in _ALLPROP})), flush=True)

_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk97_%s.out")
...
_new = _old + """
    ...
    # THE NARROW BAR (walk 97, 18 September 2026; collection 218; pre-registered before this ran).
    # The blanket bar above costs 207 days of warning across the nine peaks - 55 on 2001, 56 on 2020 - and the
    # ungated chronology at the live configuration has no false alarms, so in this sample it buys nothing. It
    # exists for one event: leg N proposed alone on 28 April 2026 and opened an episode, which withdrew walks 68,
    # 71 and 72. Leg N's proposal has no other armed proposal within ninety days, or a hundred and eighty, so a
    # SECOND-MECHANISM requirement refuses it exactly as the blanket bar does and returns the 207 days.
    # A leg may open alone only when a DIFFERENT MECHANISM also proposed within ninety days. Two proposals of the
    # same mechanism are not agreement: the mechanism map merges families, so the fifteen- and thirty-year
    # mortgage rates count once.
    def _seconded(a_, self_L):
        mine = _LEGMECH.get(self_L, self_L)
        for _L2, _pp in _ALLPROP.items():
            if _LEGMECH.get(_L2, _L2) == mine: continue
            for (_a2, _b2) in _pp:
                if abs((_a2 - a_).days) <= 90: return True
        return False
    def _admit(a_, self_L):
        return _accelerates(a_) or _seconded(a_, self_L)
    if p.get('warnw'):
        _prop = WARN_PROPOSALS.get((p['warnw'], p['warnp']), [])
        if _prop:
            _bars = _bars_from(turns)
            _keep = [(a, b) for a, b in _prop
                     if not any(s <= a <= e for s, e in _bars) and _admit(a, 'N')]
            if _keep:
                legs['N'] = _keep; _added = True
    for _L in ((p.get('newlegs') or '') + (p.get('extra') or '')):
        _prop = NEW_LEGS.get(_L, [])
        if _prop:
            _bars = _bars_from(turns)
            _keep = [(a, b) for a, b in _prop
                     if not any(s <= a <= e for s, e in _bars) and _admit(a, _L)]
            if _keep:
...
GRID = GRID + [('warnw', [26, 13, None]), ('warnp', [95, 90, 85]), ('newlegs', [None, 'A', 'AY', 'AYG', 'AYGJ', 'AYGJF']), ('extra', [None, 'c', 'm', 'cm'])]
NAMES = [n for n, _ in GRID]; GD = dict(GRID)
BASE15 = dict(BASE15); BASE15.update(warnw=None, warnp=90, newlegs=None, extra=None)
