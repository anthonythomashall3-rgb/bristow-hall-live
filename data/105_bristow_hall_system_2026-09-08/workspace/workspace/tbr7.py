...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr7.out','w')"))
def build_v(p):
...
P("THE SETTLING WINDOW, WHICH HAS NEVER BEEN SWEPT. A shorter window settles sooner because the level has less")
P("history to sit inside; too short and it settles on noise.")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
import itertools
for back,stable,band,need in itertools.product([15,18,24,30,36,48],[2,3],[0.02,0.05],[1,2]):
    c=bristow_closer('the insured rate, inverted',band,3,12,stable,back=back)
    if not c: continue
    cc=confirm_close2(c,0.10,2.0,need)
    if not cc: continue
    trow(f'  window {back}m, settles {stable}m, band {band}, {need} conf ({len(c)} calls)',{**TLH,'S':S2,'R':cc})
out.close()
