"""WALK 35 - ONE CLOCK, ACCURACY FIRST (collection 104, 8 September 2026). walk34 with the objective reordered: on each
...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk35_%s.out'%sys.argv[1],'w')"))
