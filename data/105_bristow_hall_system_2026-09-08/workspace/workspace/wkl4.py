...
exec(open('walk3.py').read().split('CACHE6={}')[0].replace("out=open('walk3_%s.out'%sys.argv[1],'w')","out=open('wkl4.out','w')"))
...
def build8(p):
    ...
    if p.get('wline2'): legs['V']=[(a,b) for a,b,c in confirm_w(leg_sv(p['wline2'],rearm=p.get('wrearm2','window')),C1+C2,'month')]
    with contextlib.redirect_stdout(io.StringIO()): turns=B.american_chronology(legs,TLH)
    return score13(turns),turns
P(f"{'W low':>7s} {'W high':>7s} {'rearm2':7s}  onsets 1948..2024                                            others")
for wl in [0.40,0.35,0.30]:
    for wh in [None,0.6,0.5,0.45]:
        for r2 in (['window'] if wh else ['-']):
            p=dict(BASE); p['wline']=wl; p['wrearm']='zero'; p['wline2']=wh; p['wrearm2']=r2
            r,t=build8(p); lp=[r['lags_p'].get(i) for i in range(13)]
            legs_used=[r['opens'][i]['leg'] if i in r['opens'] else '-' for i in range(13)]
            P(f"{wl:7.2f} {str(wh):>7s} {r2:7s}  {lp}  {[(d,lg) for d,_,lg in r['other']]}")
            P(f"        legs {legs_used}")
out.close()
