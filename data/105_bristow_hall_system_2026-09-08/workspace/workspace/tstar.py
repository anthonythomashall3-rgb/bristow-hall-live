...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tstar.out','w')"))
    ...
    for nm in (['starts','hours'] if which=='both' else [which]):
        ...
        for m,v in rel_.items():
            if armed and v>=pct:
                ...
                if not len(seg) or seg.iloc[-1]<claimdrop: continue
