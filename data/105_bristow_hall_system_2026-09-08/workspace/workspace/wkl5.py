...
exec(open('walk3.py').read().split('CACHE6={}')[0].replace("out=open('walk3_%s.out'%sys.argv[1],'w')","out=open('wkl5.out','w')"))
...
P(f"{'conf':5s} {'W low':>6s} {'W high':>7s}  onsets                                                        dates  others")
for cw in ['C1','C2','all']:
    for wl,wh in [(None,None),(0.35,None),(None,0.50),(0.35,0.50),(0.40,0.50),(0.35,0.45),(0.30,0.50)]:
        p=dict(BASE); p['wline']=wl; p['wrearm']='zero'; p['wline2']=wh; p['wrearm2']='window'; p['wconf']=cw
        r,t=build8(p); lp=[r['lags_p'].get(i) for i in range(13)]
        er=[r['errs_p'].get(i) for i in range(13)]
        P(f"{cw:5s} {str(wl):>6s} {str(wh):>7s}  {lp}  ex{sum(1 for e in er if e==0)} w1 {sum(1 for e in er if e is not None and abs(e)<=1)} worst {max([abs(e) for e in er if e is not None])}  {[(d,lg) for d,_,lg in r['other']]}")
out.close()
