...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('sahm39.out','w')"))
def build_v(p):
...
def crossings(s,line,start):
    ...
    for m,v in s.items():
        if m<start: continue
        if armed and v>=line:
            out_.append((rel.get(m,pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=4)),m)); armed=False
...
def sahm_score(nm,cr,start):
    ...
    for pub,m in cr:
        ...
        if k is None:
            ...
            used.add(k); hits.append((k,(m.year-PK[k].year)*12+m.month-PK[k].month,(pub-me(PK[k])).days))
    miss=[PK[i].strftime('%Y-%m') for i in range(13) if PK[i]>=start and i not in used]
    dd=[d for _,_,d in hits]; mm=[x for _,x,_ in hits]
    P(f"{nm}")
    P(f"   detected {len(hits)}/{len(hits)+len(miss)} missed {miss} | months after the peak, median {np.median(mm):.1f} | days after the peak month ended, median {np.median(dd):.0f}, mean {np.mean(dd):.1f} | within the month {sum(1 for x in dd if x<=31)}/{len(dd)} | FALSE POSITIVES {len(fp)} {fp}")
P("SAHM'S OWN PROTOCOL: fix the line once, read the data as published, run forward continuously, count the lag at each")
P("peak and every crossing with no recession behind it. Her published series, then this rule on the same footing.")
ST=pd.Timestamp('1959-12-01'); ST61=pd.Timestamp('1961-11-03')
sahm_score("\nSahm's own real-time indicator at 0.50, from December 1959",crossings(SR,0.50,ST),ST)
turns=full('v3.9 frozen',BASE39,TLH)
cr=[(x['published'],x['date']) for x in turns if x['kind']=='peak' and x['published']>=ST]
sahm_score("\nthe Bristow Hall Rule v3.9, read real-time, from December 1959",cr,ST)
cr61=[(a,b) for a,b in cr if a>=ST61]
sahm_score("\nthe Bristow Hall Rule v3.9, from November 1961 - the main sample under Rule 25",cr61,ST61)
out.close()
