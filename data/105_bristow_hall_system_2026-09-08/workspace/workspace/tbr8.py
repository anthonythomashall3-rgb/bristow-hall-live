...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr8.out','w')"))
def build_v(p):
...
detail('the shipped closers with S rebuilt',{**TLH,'S':S2})
c48=bristow_closer('the insured rate, inverted',0.02,3,12,3,back=48)
detail('the settling test, 48-month window, two confirmations - the safe upgrade',{**TLH,'S':S2,'R':confirm_close2(c48,0.10,2.0,2)})
c36=bristow_closer('the insured rate, inverted',0.02,3,12,3,back=36)
detail('the settling test, 36-month window, one confirmation - the fast upgrade',{**TLH,'S':S2,'R':confirm_close2(c36,0.10,2.0,1)})
out.close()
