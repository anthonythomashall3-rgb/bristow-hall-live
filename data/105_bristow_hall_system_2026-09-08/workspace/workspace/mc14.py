...
src=open('walk24.py').read().split('BASE15=dict(BASE)')[0].replace("out=open('walk24_%s.out'%sys.argv[1],'w')","out=open('mc14.out','w')")
exec(src)
...
for st in (6,8,10):
    for bw in (156,208):
        TL=dict(TLH); TL['Q']=qleg((-ICW).rolling(4).mean().dropna(),st,bw,5)
        report(f"v3.18 plus Q: weekly claims settle stable={st} back={bw}w, dated by the monthly insured rate",TL)
out.close()
