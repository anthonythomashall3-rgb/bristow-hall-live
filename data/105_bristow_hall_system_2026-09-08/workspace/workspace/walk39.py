...
Run:  python3 walk39.py 1962 2026 w39   (the diary from the 1962 cut; ~20 minutes)"""
import sys,pickle,os
exec(open('walk38.py').read().split("Y0,Y1,VAR=int(sys.argv[1])")[0].replace("walk38_%s.out","walk39_%s.out"))
...
_vfp=(_JF/_CF.reindex(_JF.index)*100).dropna(); _vfp=_vfp[_vfp.index>=pd.Timestamp('2010-07-01')]
# (18 September 2026, collection 202) the guard's bound is taken from today, never from a fixed date. It had read
# >= 2026-01-01, which passed on the day it was written and could never fire again: a vacancy first-print file frozen
# in January 2026 would have satisfied it for ever, and the hub's confirmer would have run on a dead object in silence.
# JOLTS prints a month about five weeks after it ends, so eight months behind today is far past any real delay.
assert len(_vfp)>150 and _vfp.index.max()>=(pd.Timestamp.today().normalize()-pd.DateOffset(months=8)), \
    'vacancy first prints stale: latest %s (cache/alfred_first_JTSJOL.csv, cache/alfred_first_CLF16OV.csv)'%_vfp.index.max().date()
V0=pd.concat([V0[V0.index<_vfp.index.min()],_vfp]).sort_index()
