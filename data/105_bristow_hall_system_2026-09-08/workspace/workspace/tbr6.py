...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr6.out','w')"))
def build_v(p):
...
def bristow_closer(nm,band,n,L,stable,lead=9):
    ...
    for k,m in enumerate(months):
        w1=m; w0=m-pd.DateOffset(months=36)
...
P("THE SETTLING TEST ON THE SURVEY-WEEK READING. The monthly insured rate for a month is published on the eleventh of")
P("the next; the survey-week reading is published about four days BEFORE that month ends. Same rule, faster clock.")
lagdiff=[(REL['the insured rate, inverted'](m)-REL['the survey-week insured rate, inverted'](m)).days for m in SI.index if m in SW.index]
P(f"   the survey-week clock beats the monthly one by a median of {np.median(lagdiff):.0f} days")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
trow('  + the monthly settling test, 3m, one confirmation',{**TLH,'S':S2,'R':confirm_close2(bristow_closer('the insured rate, inverted',0.02,3,12,3),0.10,2.0,1)})
import itertools
for band,stable,need in itertools.product([0.02,0.05],[2,3,4],[1,2]):
    c=bristow_closer('the survey-week insured rate, inverted',band,3,12,stable)
    if not c: continue
    cc=confirm_close2(c,0.10,2.0,need)
    if not cc: continue
    trow(f'  survey-week settling, band {band}, settles {stable}m, {need} conf ({len(c)} calls)',{**TLH,'S':S2,'R':cc})
out.close()
