...
import itertools
exec(open('fast37.py').read().split("RES={}")[0].replace("out=open('fast37.out','w')","out=open('causal25.out','w')"))
GRID=dict(sahm=[0.50], vac=[0.24,0.30,0.36,0.42], u45=[0.35,0.45,0.55,0.70], low=[0.15,0.25,0.35], starts=[25,30,35,40,45], half=[2,3,4,5])
def build(p,s):
...
def sc(p,s,tag):
    ...
    return CACHE[k]
FROZEN=dict(sahm=0.50,vac=0.36,u45=0.45,low=0.25,starts=35,half=4)
