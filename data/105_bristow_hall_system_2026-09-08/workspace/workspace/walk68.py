"""WALK 68 - LEG U REPLACES LEG S (14 September 2026).
Pre-registered in PREREG-v341-ADDENDUM-2026-09-14.md before this was run.
...
Run:  python3 walk68.py 1962 2026 w68"""
...
_SP = os.path.expanduser('~/Projects/Onset Detector Data/145_leg_U_state_insured_count_breadth/out/leg_u_proposals.csv')
S_PROPOSALS = {}
for _r in csv.DictReader(open(_SP)):
    S_PROPOSALS.setdefault(int(_r['setting']), []).append(
        (pd.Timestamp(_r['published']), pd.Timestamp(_r['dated'])))
print('leg U settings loaded:', {k: len(v) for k, v in sorted(S_PROPOSALS.items())}, flush=True)

_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk68_%s.out")
...
_new = _old + """
    ...
    if p.get('sset'):  _extra.append(('U', S_PROPOSALS.get(p['sset'], [])))
...
def obj(vw):
    ...
    a = (sum(1 for x in v if x >= 0),                          # a call on or after the peak month's end
         sum(1 for x in v if x < EARLIEST or x > LATEST),       # a call outside one-to-thirty-one days before
         float(np.median(v)),                                   # v3.29's own key, unchanged
         float(np.mean(v)))                                     # v3.29's own key, unchanged
...
GRID = GRID + [('warnw', [26, 13, None]), ('warnp', [95, 90, 85]),
               ('sset', [1, 3, 5, None])]   # safest first, by firing count
