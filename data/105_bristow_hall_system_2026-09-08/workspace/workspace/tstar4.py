...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tstar4.out','w')"))
    ...
    for nm in (['starts','hours'] if which=='both' else [which]):
        ...
        for m,v in rel_.items():
            if armed and v>=pct:
                ...
                if len(seg) and seg.iloc[-1]<claimdrop: continue
                # DATING: the demand object's own low is NOT the trough - starts bottom months before it, which is
                # exactly why they are fast. The close is therefore published on the demand reading and DATED at the
                # month the claims flow peaked, which is what a trough is, and which is known on the day of publication.
                cw=CL[(CL.index>=pub-pd.DateOffset(months=18))&(CL.index<=pub)]
...
def score(tag,TL):
    ...
    if len(rows)<13:
        RES.append(dict(tag=tag,med=0,worst=0,w31=0,w31b=0,ex=0,w1=0,prem=len(prem),oth=len(oth),fa=99,det=len(rows),lags=[],rows=rows)); return None
  ...
  for pct in [0.5,1.0,2.0,3.0]:
    for look in [12,18,24]:
      for cd in [0.02,0.05,0.10,0.15,0.20]:
        ...
        if not cc: continue
        TL=dict(TLH); TL['Q']=cc
        score(f'{which} +{pct}% over {look}m low, claims {cd} off peak',TL)
