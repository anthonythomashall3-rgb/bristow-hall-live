...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tconj3.out','w')"))
p0=dict(BASE)
...
P("WHICH CONFIRMER BINDS EACH CLOSE, and what each demand object could offer")
for d,fr in [(0.10,1.00),(0.06,1.00)]:
    cf=confirm_t(prop_weekly(d),fr)
    P(f"\nclaims drop {d}, retrace {fr}")
    for pp,dd,nm in cf:
        i=[k for k in range(13) if TR[k]-pd.DateOffset(months=6)<=dd<=TR[k]+pd.DateOffset(months=12)]
        if not i: continue
        i=i[0]; raw=[q for q in prop_weekly(d) if q[1]==dd]
        rawp=raw[0][0] if raw else None
        det=[]
        for c in REC:
            ser=c['gap']; wors=ser[(ser.index>=dd-pd.DateOffset(months=18))&(ser.index<=dd)]
            if not len(wors) or wors.max()<=c['line']: det.append(f"{c['name']}: never crossed"); continue
            mx=wors.max(); tgt=mx-(mx-c['line'])*fr
            seg=ser[(ser.index>=dd)&(ser.index<=dd+pd.DateOffset(months=6))]; hit=seg[seg<=tgt]
            det.append(f"{c['name']}: {pubof2(c,hit.index[0]):%Y-%m-%d}" if len(hit) else f"{c['name']}: no retrace")
        P(f"   {TR[i]:%Y-%m}  close {pp:%Y-%m-%d} ({(pp-me(TR[i])).days:+d}d) by {nm} | proposal was ready {rawp:%Y-%m-%d} ({(rawp-me(TR[i])).days:+d}d)")
        P(f"        "+" | ".join(det))
out.close()
