...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc6.out','w')"))
def build_v(p):
...
def confirm_two_sided(calls,cdrop,rise,need,fwd=6):
    ...
    for pp,dd in calls:
        ...
        if len(hits)>=need:
            hits.sort(); out_.append((max(pp,hits[need-1]),dd))
    return out_
P("THE STARTS CLOSER GUARDED BY THE CLAIMS FLOW. A demand object settling is not by itself a labour-market close.")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
base={**TLH,'S':S2}
...
trow('  + the insured rate settling',{**base,'R':R1})
import itertools
for stable,cd,need in itertools.product([4,6,8],[0.15,0.20,0.25,0.30],[1,2,3]):
    c=first_to_settle(stable=stable,back=48,chans=['housing starts'])
    if not c: continue
    cc=confirm_two_sided(c,cd,2.0,need)
    if not cc: continue
    trow(f'  + starts settling {stable}m, claims {cd} off peak, {need} agree ({len(cc)} calls)',{**base,'R':R1,'T':cc})
out.close()
