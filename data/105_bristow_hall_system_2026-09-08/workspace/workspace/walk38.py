"""WALK 38 - WALK 37 WITH THE HOURS PAIR DATED BY THE EMPLOYMENT SITUATION RELEASE CALENDAR (Rule Zero, V321 note
...
WITH THE PAPER SPREAD CORRECTED AFTER AUGUST 1997 (Rule Zero, collection 105, 8 September
...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk38_%s.out'%sys.argv[1],'w')"))
...
def _settle_starts(band=0.02,n=3,L=12,stable=4,back=48):
    ...
    # (17 September 2026, collection 201) the last month comes from the data, never from a fixed date. The live chain
    # overrides this first-print closer with s2/asof_trough.py (whose own fixed '2026-08-01' was corrected the same day),
    # so nothing live read this bound; any run of the chain without asof_trough would have lost closer T after August 2026.
    for m in pd.date_range('1948-01-01',max(pd.Timestamp('2026-08-01'),(pd.Timestamp(_LHs.index.max())+pd.DateOffset(months=1)).replace(day=1)),freq='MS'):
