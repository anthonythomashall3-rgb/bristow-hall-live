...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tfast2.out','w')"))
        ...
        for nmin in [1,2,3]:
          for useslow in [False,True]:
            for spread_on in [False,True]:
              ...
              if prem or oth: continue
              CAND.append((len(rows),sum(1 for i,(l,e) in rows.items() if l<=31),sum(1 for i,(l,e) in rows.items() if i>=4 and l<=31),
                           rows,f'drop{drop} minw{minw} fall{int(falling)} rise{pct} n{nmin} slow{int(useslow)} spr{int(spread_on)}',cc))
P(f"closer-only candidates with no premature close and nothing outside a window: {len(CAND)}")
...
P("now the best twenty through the whole chronology")
BEST=[]
def score(tag,TL):
