...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc4.out','w')"))
def build_v(p):
...
P("EVERY CHANNEL GIVEN THE SETTLING TEST, THE FIRST TO SETTLE CLOSING")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
trow('  the single-channel test, 48m/3m/2conf',{**TLH,'S':S2,'R':confirm_close2(bristow_closer('the insured rate, inverted',0.02,3,12,3,back=48),0.10,2.0,2)})
import itertools
for stable,back,need in itertools.product([4,5,6,8],[36,48],[1,2]):
    c=first_to_settle(stable=stable,back=back)
    if not c: continue
    cc=confirm_close2(c,0.10,2.0,need)
    if not cc: continue
    trow(f'  first to settle, {stable}m settle, {back}m window, {need} conf ({len(c)} calls)',{**TLH,'S':S2,'R':cc})
out.close()
