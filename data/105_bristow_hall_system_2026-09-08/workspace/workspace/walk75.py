...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk75_%s.out")
...
_new = _old + """
    def _bars_from(turns):
        ...
        return bars
    _added = False
    # The arming bar as the pre-registrations state it, rather than as it was implemented. A leg
    # proposal survives only if the CORE chronology -- built with no leg at all -- opens within four
    # hundred days after it. The leg moves a call the core already makes earlier; it cannot make a
    # call the core does not make. This walk is the safety check on that change: walk 69's record is
    # the standing best, and if the fix is sound the record here is identical to it.
