"""WALK 33 - ONE CLOCK FOR THE CHRONOLOGY, THE LINES CHOSEN BY SPEED (collection 104, 8 September 2026). walk32 with
...
WALK 32 - ONE CLOCK, THE MENU THAT SURVIVES IT (collection 104, 8 September 2026). Three changes to walk31.
...
WALK 31 - ONE CLOCK (collection 104, 8 September 2026). Anthony: "When the rule fires, that is the date. Nothing more
...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk33_%s.out'%sys.argv[1],'w')"))
...
def closer_C(D,n,s,A=20.0,c=3.0,u=3,need=1,k=1,pub_cc=12,cool=26):
    ...
    for i in ok:
        ...
        calls.append((pub,dated)); last_=t   # one clock: no hold; the date is overridden by the month of publication
    return calls
