...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('phl2.out','w')"))
def build_h(p):
...
P(f"{'hline':>6s}  onsets 1948..2024                                              within month  others")
for hl in [1.0,0.95,0.9,0.85,0.8,0.75,0.7,0.6]:
    ...
    for x in t:
        ...
        if x['published'].strftime('%Y-%m-%d') not in [d for d,_,_ in r['other']]: continue
        inrec=any(pk-pd.DateOffset(months=6)<=x['date']<=tr for pk,tr in zip(PK,TR))
        oth.append((x['published'].strftime('%Y-%m-%d'),x['date'].strftime('%Y-%m'),x['leg'],'INSIDE a recession' if inrec else 'FALSE ALARM'))
    P(f"{hl:6.2f}  {lp}  {sum(1 for x in v if x<=31)}/13, from1961 {sum(1 for x in v9 if x<=31)}/9")
    for o in oth: P(f"          second call {o[0]} dated {o[1]} by {o[2]} - {o[3]}")
out.close()
