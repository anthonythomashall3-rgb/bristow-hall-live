...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk82_%s.out")
...
def obj(vw):
    v, w = vw
    # WALK 82: THE BAND NARROWED, AND NOTHING ELSE. The target is a rule whose calls all land between
    # one day and roughly one month early -- consistent timing rather than leads scattered from -2 to
    # -76. This narrows the counting key's window from [-92, -1] to [-31, -1] and keeps v3.29's own
    # median and mean of the SIGNED lead as the tie-breaks, untouched.
    # WHY A COUNTING KEY AND NOT A DISTANCE KEY. Walk 79 tried to pull the calls toward -1 with a
    # distance key -- the median of |lead + 1| -- and it failed badly: three late calls, a false alarm
    # in May 2010, and the spread WIDENED from 22 days to 43. That is the fourth time a distance key has
    # damaged this record, after walks 56, 58 and 64. Counting keys have the opposite history: walk 78's
    # is what took the scored record to eight of eight. So this changes the band the counting key
    # counts, and adds nothing.
    # THE RISK, NAMED BEFORE THE RUN. A call at -31 is thirty days from being late and a call at -76 is
    # seventy-five. Narrowing the band deliberately removes margin, and the same pressure that pushes a
    # call from -76 to -31 can push another from -2 to +3. The false-alarm and late-call conditions are
    # what that would trip, and on walk 79's evidence it is the likeliest outcome.
    a = (sum(1 for x in v if x > -1 or x < -31), float(np.median(v)), float(np.mean(v)))
