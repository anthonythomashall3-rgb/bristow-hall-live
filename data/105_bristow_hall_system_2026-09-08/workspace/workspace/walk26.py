...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk26_%s.out'%sys.argv[1],'w')"))
...
BASE15=dict(BASE); BASE15.update(rst=6,tst=4,qst=8,deep=999,wline=None,wline2=None,bshare=None,hline=1.00,spr=round(LINE,3),sahm=0.43,vl=0.20,hback=6)
GRID=[('sahm',[0.55,0.50,0.45,0.43,0.40,0.3667,0.35]),('vl',[0.36,0.30,0.25,0.20,0.15,0.12]),('hline',[1.10,1.00,0.95,0.90,0.85,0.80]),
      ...
      ('look',[52,78,91,130]),('ic',[60,50,45]),('rst',[8,6,5,4]),('tst',[6,5,4,3]),('qst',[13,10,8,6]),
...
# THE TROUGH ANNOUNCEMENTS. Rule Zero, 7 September 2026: this line's pre-1980 trough announcement dates were
# invented and have been withdrawn, so no trough before July 1980 can be ratified and the walk necessarily holds
# the safest run lengths until the summer of 1981. The dates below are the committee's own memoranda.
TANN={'1980-07':'1981-07-08','1982-11':'1983-07-08','1991-03':'1992-12-22','2001-11':'2003-07-17',
...
SUMF='cache/w26_sum.pkl'; SUM=pickle.load(open(SUMF,'rb')) if os.path.exists(SUMF) else {}
def summary(p):
...
def clean(q,cut,ks,kt=()):
    ...
    for i in kt:
        ...
        pub_,lg,er=s['pair'][i]
        if lg<0 or abs(er)>1: return None                  # premature, or dated more than a month out
        w.append(lg)
    return (v,w)
def obj(vw):
    v,w=vw
    ...
    return a+(sum(1 for x in w if x>31), float(np.median(w)), float(np.mean(w)))
def neighbours(p):
    for n in NAMES:
...
for Y in range(Y0,Y1+1):
    ...
    for _ in range(2):
        ...
        for name,grid in GRID:
            ...
            p[name]=nv
        if not ch: break
    if clean(p,cut,ks,kt) is None: continue
