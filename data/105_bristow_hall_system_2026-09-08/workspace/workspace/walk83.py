...
_hdr = open('walk54.py').read().split(_MARK)[0].replace("walk54_%s.out", "walk83_%s.out")
...
def obj(vw):
    v, w = vw
    # WALK 83: the target narrowed to UNDER TWO MONTHS. The counting key's window becomes [-61, -1].
    # Walk 81 has three calls outside it -- 1981 at -64, 1990 at -76 and 2024 at -88 -- and six inside.
    # This is the middle of three bands now tested: [-92,-1] held with zero late calls (walk 78/81),
    # [-31,-1] produced two late calls (walk 82). If the failure is gradual rather than sudden, [-61,-1]
    # is where it shows.
