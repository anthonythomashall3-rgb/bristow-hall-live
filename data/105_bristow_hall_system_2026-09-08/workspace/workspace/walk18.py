...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk18_%s.out'%sys.argv[1],'w')"))
def build_v(p):
...
GRID=[('sahm',[0.55,0.50,0.45,0.43,0.40,0.3667,0.35]),('vl',[0.36,0.30,0.25,0.20,0.15,0.12]),('hline',[1.10,1.00,0.95,0.90,0.85,0.80]),
      ('spr',[1.9,1.6,round(LINE,3),1.1,0.9]),('hback',[6,5,4,3]),('u45',[0.70,0.55,0.45,0.35,0.30]),('low',[0.35,0.30,0.25,0.20,0.15]),
...
SUMF='cache/w18_sum.pkl'; SUM=pickle.load(open(SUMF,'rb')) if os.path.exists(SUMF) else {}
def summary(p):
