...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('pW.out','w')"))
def build_v(p):
...
def full4(nm,p,TL,nvac=1):
    ...
    P(f"     {lp}")
P("the vacancy in the hub's window at the 2024 call and at the December 2025 call")
p=dict(BASE39); G=vgap2(4,4)
for m,lab in [(pd.Timestamp('2024-04-01'),'2024 (Sahm month 2024-04, dated 2024-01)'),(pd.Timestamp('2025-11-01'),'2025 (Sahm month 2025-11, dated 2025-08)')]:
    w=G[(G.index>=m-pd.DateOffset(months=6))&(G.index<=m)]
    P(f"   {lab}: "+", ".join(f"{k:%Y-%m}:{v:.3f}" for k,v in w.items())+f"  -> months at or above 0.20: {int((w>=0.20).sum())}")
P("")
P("REQUIRING THE VACANCY TO HOLD ITS LINE FOR MORE THAN ONE MONTH")
TLS=dict(TLH); TLS['S']=confirm_close(TLG['S'],0.15,2.0,need=2)
for nv in [1,2,3]:
    full4(f'  vacancy must hold for {nv} month(s)',BASE39,TLS,nv)
out.close()
