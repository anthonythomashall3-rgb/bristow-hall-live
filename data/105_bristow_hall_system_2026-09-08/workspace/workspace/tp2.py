...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tp2.out','w')"))
def build_v(p):
...
def prop_weekly2(drop,minw=8,pub=5):
    ...
    return out_
def confirm_hold(calls,pct,nmin,hold,fwd=6):
    """THE RULE THAT MADE THE HUB SAFE, APPLIED TO THE TROUGH SIDE: a recovery object must hold its ground for `hold`
    months, not merely touch it once. The premature closes of 1970 and 1982 were single-month readings."""
    objs=[('starts',RSc,18),('hours',RHc,5)]
    out_=[]
    for pp,dd in calls:
        hits=[]
        for nm,rl_,pdy in objs:
            seg=rl_[(rl_.index>=dd)&(rl_.index<=dd+pd.DateOffset(months=fwd))]
            h=seg[seg>=pct]
            if len(h)>=hold: hits.append(pd.Timestamp(h.index[hold-1].year,h.index[hold-1].month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=pdy-1))
        seg=BELOWc[(BELOWc.index>=dd)&(BELOWc.index<=dd+pd.DateOffset(months=fwd))]
        h=seg[seg>=0.10]
        if len(h)>=hold*4: hits.append(h.index[hold*4-1]+pd.Timedelta(days=5))
        if len(hits)>=nmin:
            hits.sort(); out_.append((max(pp,hits[nmin-1]),dd))
    return out_
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
P("THE TWO-MONTH HOLD APPLIED TO THE TROUGH SIDE. The premature closes were single-month readings; requiring a")
P("recovery object to hold its ground should make the fast closer safe the way it made the hub safe.")
trow('  the shipped closers with S rebuilt',{**TLH,'S':S2})
import itertools
for drop,pct,nmin,hold in itertools.product([0.15,0.12,0.10,0.08,0.06],[1.0,2.0,3.0],[1,2],[1,2,3]):
    q=confirm_hold(prop_weekly2(drop),pct,nmin,hold)
    if not q: continue
    trow(f'  claims drop {drop}, rise {pct}% held {hold}m, {nmin} needed',{**TLH,'S':S2,'Q':q})
out.close()
