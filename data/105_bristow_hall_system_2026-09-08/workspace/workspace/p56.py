...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('p56.out','w')"))
def build_v(p):
    ...
    if p.get('sline'):
        # housing STARTS on their own, not the pair: the three-month mean below its own twelve-month maximum by a
        # given fraction, published on the starts release. The pair does not exist until 1961; starts do, from 1959.
        mm=lh.rolling(3).mean(); ss=((lh.rolling(12).max()-mm)/lh.rolling(12).max()*100).dropna()
...
OUT2=open("p56.out","w")
def P(*a):
    print(*a); print(*a,file=OUT2); OUT2.flush()
...
OUT2.close()
BASE39=dict(BASE38); BASE39['hback']=5; BASE39['vl']=0.20
P("housing STARTS on their own as a confirmer - they exist from 1959, the pair only from 1961")
show('  v3.9 for reference',BASE39)
for sl_ in [30,25,20,17,15,12,10]:
    p=dict(BASE39); p['sline']=sl_
    show(f'  starts alone {sl_} per cent below their 12-month max',p)
OUT2.close()
