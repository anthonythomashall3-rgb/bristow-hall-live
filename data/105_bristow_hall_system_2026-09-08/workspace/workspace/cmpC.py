...
src=open('walk25.py').read().split('BASE15=dict(BASE)')[0].replace("out=open('walk25_%s.out'%sys.argv[1],'w')","out=open('cmpC.out','w')")
exec(src)
    ...
    for i in range(warm,len(Dv)):
        ...
        if state=='closed':
            if float(Dv.iloc[i])>=threshold:
                ...
                dt=B._median(dates)
                onset=pd.Timestamp(dt.year,dt.month,1); calls.append((pub(t),onset,'peak')); state='open'; start=onset
        else:
