...
exec(open('fast41.py').read().split("def full(")[0].replace("out=open('fast41.out','w')","out=open('sweep11.out','w')"))
sys.path.insert(0,W+'/lab/weekly'); import american_chronology as A
VG={}
def vgap(k,back):
    if (k,back) not in VG: VG[(k,back)]=A.vacancy_gap_rt(k,back)
    return VG[(k,back)]
P("identity check: the lab's vacancy_gap_rt(2,6) against the shipped vr, max |diff| =",float((vgap(2,6).reindex(vr.index)-vr).abs().max()))
