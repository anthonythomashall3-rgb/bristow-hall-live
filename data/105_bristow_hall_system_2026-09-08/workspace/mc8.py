...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc8.out','w')"))
def build_v(p):
...
detail('the shipped closers with S rebuilt',base)
c=first_to_settle(stable=4,back=48,chans=['housing starts'])
T1=confirm_ur(c,1,0.0)
detail('+ the insured rate settling, + housing starts guarded by the unemployment rate rolling over',{**base,'R':R1,'T':T1})
out.close()
