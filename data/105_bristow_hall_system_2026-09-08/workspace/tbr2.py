...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr2.out','w')"))
def build_v(p):
...
def bristow_closer(nm,band,n,L,stable,lead=9):
    ...
    return res
P("THE BRISTOW RULE AS THE CLOSER, WITH THE CONFIRMATION THAT FIXED S")
P("On the insured rate it puts six of thirteen closes inside the month against the shipped closers' two, and costs")
P("one close outside a trough window. S had exactly that fault and two confirmations removed it.")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  the shipped closers with S rebuilt',{**TLH,'S':S2})
import itertools
for band,stable in itertools.product([0.02,0.05],[2,3]):
    c=bristow_closer('the insured rate, inverted',band,3,12,stable)
    if not c: continue
    trow(f'  Bristow on the insured rate, band {band}, settles {stable}m, unconfirmed',{**TLH,'S':S2,'R':c})
    for cd,rise,need in itertools.product([0.10,0.15],[1.0,2.0],[1,2]):
        cc=confirm_close2(c,cd,rise,need)
        if not cc: continue
        trow(f'  the same, claims {cd} or a demand object +{rise}%, {need} needed',{**TLH,'S':S2,'R':cc})
out.close()
