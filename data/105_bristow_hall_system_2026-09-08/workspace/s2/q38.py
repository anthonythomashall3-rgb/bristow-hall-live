...
# Run: PYTHONPATH=. python3 s2/q38.py   (loop variables carry underscores: a bare `rel` would shadow the lab's release calendar, as `g` once did)
...
for tag,(_day,_g7,_rel) in GT_TERMS.items():
    alone=[]; armed=True
    for t,v in _rel.items():
        d=t+pd.Timedelta(days=1)
        if armed and v>=p['kc'][0]-EPS: alone.append(d); armed=False
        elif not armed and v<=EPS: armed=True
    gated=[d for d,m in leg_K_search_one(_rel,p['kc'][0],p['kc'][1])]
    print(f"{tag}: history {_day.index.min().date()}..{_day.index.max().date()} | alone fires {len(alone)}, outside a recession window {len([d for d in alone if not inwin(d)])} | with the gate {[d.date().isoformat() for d in gated]} outside {[d.date().isoformat() for d in gated if not inwin(d)]}")
    print('   March 2020 (datum day -> reading):',{t.strftime('%m-%d'):round(float(v),1) for t,v in _rel['2020-03-08':'2020-03-16'].items()})
