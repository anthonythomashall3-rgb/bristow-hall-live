...
DRIVERS = ['bhs_run.sh', 'bhs_build.py', 'bhs_site.py', 'bhs_update.py', 'bhs_schedule.py', 'bhs_dol_press.py',
           ...
           's2/walk_health.py', 's2/versions.py', 's2/shape_guard.py', 's2/trace_reads.py',
           's2/cloud_bundle.py']
...
def collect():
    ...
    data = {}
    # THE OBSERVED READ SET. The patterns above are seven directory globs written by hand, and on 18 September the
    # cloud runner found files outside them one at a time until s2/trace_reads.py watched a real run and counted
    # 111 inputs the freeze had never hashed. A hand-written list of what a program reads will always have holes;
    # a traced one does not. out/read_set.json is regenerated by s2/trace_reads.py and folded in here, so the
    # freeze covers what the rule ACTUALLY reads rather than what someone remembered it reading.
    try:
        _rs = json.load(open(os.path.join('out', 'read_set.json')))
        for _p in _rs.get('paths') or []:
            _f = _p if os.path.isabs(_p) else os.path.join(ROOT, _p)
            if os.path.isfile(_bridged(_f)):
                data[os.path.relpath(_f, ROOT) if _f.startswith(ROOT) else _f] = sha(_f)
    except Exception:
        pass
