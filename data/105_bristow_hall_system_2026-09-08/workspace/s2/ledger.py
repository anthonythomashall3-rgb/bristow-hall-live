...
"""
import os, sys, json, hashlib, datetime, subprocess, glob, time   # glob, time: v3.69 (22 September 2026, collection 302)
...
def upgrade(limit=30, min_age_hours=6):
    """v3.69 (22 September 2026, collection 302; the live-ops audit): upgrade the OpenTimestamps receipts still pending in the Bitcoin chain. A
    receipt is complete once `ots upgrade` says so; complete receipts are listed in live/ots/COMPLETE.txt and never touched
    again. Ages come from the receipt's name (the line's own UTC stamp), not from file times, which a checkout resets."""
    d = os.path.join(COL, 'live', 'ots'); done_f = os.path.join(d, 'COMPLETE.txt')
    if not os.path.isdir(d): return 'no receipts'
    done = set(open(done_f).read().split()) if os.path.exists(done_f) else set()
    now = datetime.datetime.now(datetime.timezone.utc); todo = []
    for f in sorted(glob.glob(os.path.join(d, '*.ots'))):
        b = os.path.basename(f)
        if b in done: continue
        try: at = datetime.datetime.strptime(b[:16], '%Y%m%dT%H%M%SZ').replace(tzinfo=datetime.timezone.utc)
        except Exception: continue
        if (now - at).total_seconds() >= min_age_hours * 3600: todo.append(f)
    newly = []
    for f in todo[:limit]:
        try:
            r = subprocess.run(['ots', 'upgrade', f], capture_output=True, text=True, timeout=60)
        except FileNotFoundError:
            return 'the ots client is not installed'
        except Exception:
            continue
        if 'Timestamp complete' in (r.stdout + r.stderr): newly.append(os.path.basename(f))
        for bak in glob.glob(f + '.bak'):
            try: os.remove(bak)
            except Exception: pass
    if newly:
        with open(done_f, 'a') as fh: fh.write('\n'.join(newly) + '\n')
    return '%d pending receipt(s) tried, %d complete now, %d complete before' % (min(len(todo), limit), len(newly), len(done))
...
def main():
    ...
    try: print('receipts:', upgrade())   # v3.69 (22 September 2026, collection 302)
    except Exception as e: print('receipts: upgrade skipped (%s)' % type(e).__name__)
    ...
    rec = {
        ...
        'commit': os.environ.get('GITHUB_SHA') or None, 'run': os.environ.get('GITHUB_RUN_ID') or None,   # v3.69 (22 September 2026, collection 302)
    }
