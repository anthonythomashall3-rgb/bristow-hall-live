...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('phl.out','w')"))
def build_h(p):
...
for hl in [1.0,0.95,0.9,0.85,0.8,0.75,0.7,0.6]:
    ...
    P(f"{hl:6.2f}  {lp}  {sum(1 for x in v if x<=31)}/13, from1961 {sum(1 for x in v9 if x<=31)}/9  {[(d,lg) for d,_,lg in r['other']]}")
out.close()
