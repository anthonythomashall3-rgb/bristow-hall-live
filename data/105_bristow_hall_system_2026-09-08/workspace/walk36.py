"""WALK 36 - WALK 34 WITH THE MENU DATED (Rule 23 clause 3; collection 104, 8 September 2026). Each object enters the
...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk36_%s.out'%sys.argv[1],'w')"))
...
def build_v(p):
    G=vgap2(p['vk'],p['vb']); G=G[G.index>=pd.Timestamp('2000-12-01')]   # the menu dated: the vacancy from JOLTS
    pubs=pd.Series({m:(relJ[m] if m in relJ.index else pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=29)) for m in G.index})
