...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('tbr4.out','w')"))
def build_v(p):
...
def bristow_weekly(band=0.02,n=4,L=52,stable=3,back_w=156,pub=5):
    ...
    return out_
P("THE SETTLING TEST ON THE WEEKLY SERIES, and several objects settling together")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
cm=bristow_closer('the insured rate, inverted',0.02,3,12,3)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
trow('  + the monthly settling test, one confirmation',{**TLH,'S':S2,'R':confirm_close2(cm,0.10,2.0,1)})
import itertools
for band,n,stable,back in itertools.product([0.02,0.05],[4,8],[3,5,8],[104,156]):
    cw=bristow_weekly(band,n,52,stable,back)
    if not cw: continue
    for need in [1,2]:
        cc=confirm_close2(cw,0.10,2.0,need)
        if not cc: continue
        trow(f'  weekly settling band {band} mean {n}w settles {stable}w back {back}w, {need} conf ({len(cw)} calls)',{**TLH,'S':S2,'R':cc})
out.close()
