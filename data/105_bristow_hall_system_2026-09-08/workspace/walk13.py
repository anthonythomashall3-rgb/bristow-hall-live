...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('walk13_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')"))
BASE13=dict(BASE); BASE13['deep']=15; BASE13['wline']=None; BASE13['wline2']=None; BASE13['bshare']=None
...
SUMF='cache/w13_sum.pkl'; SUM=pickle.load(open(SUMF,'rb')) if os.path.exists(SUMF) else {}
def summary(p):
