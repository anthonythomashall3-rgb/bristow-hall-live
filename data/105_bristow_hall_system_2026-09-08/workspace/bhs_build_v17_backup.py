...
(walk38: walk37 with the hours pair on the release calendar; identical lines at every cut) produced; (4) every object's latest reading against its line, appended to the forward log LIVE_LOG_v321.tsv
...
def score_rows(P):
    ...
    for t in idx:
        ...
        if not np.isnan(pv):
            ...
            sc=min(pv,float(w.iloc[-2]) if len(w)>=2 else np.nan)      # the hub's hold: the vacancy at its line in TWO months of the window
...
pg=pickle.load(open('cache/w38_prog.pkl','rb')); LOG=sorted(pg['log'],key=lambda z:z[0])
epis=[]; cur=None
...
for e in epis:                                   # the 2020 call: the diary's 5 May 2020 was the hours pair's approximate day
...
# dates the spread a day after its Friday, a conservative convention; the page carries the Friday, when the week's
# H.15 data exist.)
...
def _reading(P,d):
    ...
    if not np.isnan(pv):
        sec=_asof(CONF['vac'],d,p['hback'],second=True)
        if not np.isnan(sec):
            sc=min(pv,sec)
...
add('confirm','paper spread, 13-week mean above its 39-week low, points',cS*p['spr'],p['spr'],nxt='daily')
...
add('close','S&P 500 above its 26-week low on the release day, percent',pd.Series(sp_,index=_CW),float(S_),nxt='daily')
...
NEXT.append(dict(date='daily',what='S&P 500 close; commercial paper and bill rates (H.15)'))
...
SAHM=g.dropna(); SAHM=SAHM[SAHM.index>=SITE_START]
ANNS=[]
for e in epis:
...
# ---- the Sahm rule in real time, for the same rows: the first month at or above 0.50 on first prints, on its release day ----
SAHM_CALLS=[]; _armed=True
for m,v in g.dropna().items():
...
state=dict(built=str(today),next_releases=NEXT,calendar=CAL,calendar_dated_through=(_dated_through.isoformat() if _dated_through else None),announcements=ANNS,sahm_other=SAHM_OTHER,sahm=dict(dates=[t.strftime('%Y-%m') for t in SAHM.index],values=[round(float(v),3) for v in SAHM.values]),version='v3.21',lines={k:(None if v is None else v) for k,v in p.items()},standing=standing,
           ...
           through=dict(claims=lastv(ICfp)[1],insured_rate=lastv(spl)[1],unemployment_rate=lastv(g)[1],vacancy=lastv(G)[1],sp500=lastv(_SPX)[1],spread=lastv(GSP)[1]))
