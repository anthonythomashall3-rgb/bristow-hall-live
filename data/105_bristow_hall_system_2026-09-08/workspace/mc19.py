...
src=open('mc14.py').read().split('def build_with(')[0].replace("out=open('mc14.out','w')","out=open('mc19.out','w')")
exec(src)
...
for dater,other in [('starts','insured'),('insured','starts'),('starts','hours'),('hours','starts'),('insured','hours'),('hours','insured')]:
    for st in (8,10,13):
        TL=dict(TLH); TL['Q']=qleg3(_IC4,st,156,5,dater,other)
        report(f"Q: weekly claims stable={st}, dated by {dater}, corroborated by {other} within one month",TL)
out.close()
