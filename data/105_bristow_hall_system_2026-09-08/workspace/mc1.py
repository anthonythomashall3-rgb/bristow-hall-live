...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc1.out','w')"))
def build_v(p):
...
def multichannel_closer(band=0.02,n=3,L=12,stable=3,back=48,q=0.5,minch=3):
    ...
    return out_
P("THE REAL BRISTOW RULE: EVERY CHANNEL DATED, THE ORDER STATISTIC TAKEN")
P(f"   channels: "+", ".join(f"{nm} (day {d})" for nm,(lv,d) in CH.items()))
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
trow('  shipped closers, S rebuilt',{**TLH,'S':S2})
trow('  + the single-channel settling test',{**TLH,'S':S2,'R':confirm_close2(bristow_closer('the insured rate, inverted',0.02,3,12,3,back=48),0.10,2.0,2)})
import itertools
for q,stable,back in itertools.product([0.5,0.6,0.7],[2,3,4],[36,48]):
    c=multichannel_closer(stable=stable,back=back,q=q)
    if not c: continue
    trow(f'  multi-channel, q={q}, settles {stable}m, window {back}m ({len(c)} calls)',{**TLH,'S':S2,'R':c})
out.close()
