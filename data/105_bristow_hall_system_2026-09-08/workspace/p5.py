...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('p5.out','w')"))
def build_h(p):
...
P("the 2025 call fires on a vacancy reading of exactly 0.200 in May 2025. Raising the vacancy line kills it - does 2024 survive?")
import itertools
for sl,vl_ in itertools.product([0.3667,0.35,0.40,0.43],[0.20,0.22,0.25,0.28,0.30]):
    p=dict(BASE); p['deep']=999; p['wline']=0.30; p['wline2']=0.60; p['bshare']=0.50; p['hline']=0.95; p['spr']=1.1; p['sahm']=sl; p['vl']=vl_
    r,t=build_h(p); lp=[r['lags_p'].get(i) for i in range(13)]
    fa=[]
    for x in t:
        if x['kind']!='peak' or x['published'].strftime('%Y-%m-%d') not in [d for d,_,_ in r['other']]: continue
    ...
    tag="CLEAN" if (len(v)==13 and not fa) else ("misses "+str(13-len(v)) if len(v)<13 else "FA "+str(fa))
    P(f"hub {sl:6.4f} vac {vl_:4.2f}  {lp}  within {sum(1 for x in v if x<=31)}/13, from1961 {sum(1 for x in v9 if x<=31)}/9 | exact {sum(1 for e in er if e==0)} w1 {sum(1 for e in er if e is not None and abs(e)<=1)} | {tag}")
out.close()
