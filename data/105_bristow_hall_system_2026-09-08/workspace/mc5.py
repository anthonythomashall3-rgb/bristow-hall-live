...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc5.out','w')"))
def build_v(p):
...
P("HOUSING STARTS GIVEN THE SETTLING TEST, ADDED AS A SECOND CLOSER. It settles the 2024 trough at July 2024,")
P("published 17 September 2024 - seventeen days after the trough month ended, against the 140 days S takes.")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
base={**TLH,'S':S2}
...
import itertools
for stable,back,need in itertools.product([4,5,6,8],[36,48],[1,2]):
    c=first_to_settle(stable=stable,back=back,chans=['housing starts'])
    if not c: continue
    cc=confirm_close2(c,0.10,2.0,need)
    if not cc: continue
    trow(f'  + starts settling {stable}m, window {back}m, {need} conf ({len(c)} calls)',{**base,'R':R1,'T':cc})
out.close()
