...
exec(open('walk9.py').read().split('BASE9=dict(BASE)')[0].replace("out=open('walk9_%s_%s.out'%(sys.argv[3],sys.argv[1]),'w')","out=open('mc7.out','w')"))
def build_v(p):
...
def multichannel_closer(band=0.02,n=3,L=12,stable=3,back=48,q=0.5,minch=3):
    ...
    return out_
def first_to_settle(band=0.02,n=3,L=12,stable=5,back=48,chans=None):
    """EVERY CHANNEL GIVEN THE SETTLING TEST, AND THE FIRST ONE TO SETTLE CLOSES. The median waits for half the panel;
    the earliest settled channel is the closer analogue of the onset side, where any leg may propose and the
    chronology takes the first. Housing starts settle the 2024 trough at plus seventeen days; the insured rate never
    settles there at all."""
    SM={nm:(lv.rolling(n).mean().dropna(),d) for nm,(lv,d) in CH.items() if chans is None or nm in chans}
    out_=[]
    for nm,(lv,d) in SM.items():
        run=None; runlen=0; seen=set()
        for m in pd.date_range('1948-01-01','2026-08-01',freq='MS'):
            avail=lv[lv.index<=m-pd.DateOffset(months=1)]
            if len(avail)<8: continue
            dt=ch_date(avail,m-pd.DateOffset(months=back),avail.index[-1],band,1,L)
            if dt is None: run=None; runlen=0; continue
            mm=pd.Timestamp(dt.year,dt.month,1)
            if run==mm: runlen+=1
            else: run=mm; runlen=1
            if runlen>=stable and mm not in seen:
                seen.add(mm); out_.append((_pub(nm,avail.index[-1]),mm))
    out_.sort(); return out_
def confirm_two_sided(calls,cdrop,rise,need,fwd=6):
    """a close on a demand object is not a labour-market close: the claims flow must be off its own peak by `cdrop`
    AND `need` objects must agree"""
    out_=[]
    for pp,dd in calls:
        if dd<BELOWc.index.min(): continue
        seg=BELOWc[(BELOWc.index>=dd)&(BELOWc.index<=dd+pd.DateOffset(months=fwd))]
        h=seg[seg>=cdrop]
        if not len(h): continue
        hits=[h.index[0]+pd.Timedelta(days=5)]
        for rl_,pdy in [(RSc,18),(RHc,5)]:
            s2=rl_[(rl_.index>=dd)&(rl_.index<=dd+pd.DateOffset(months=fwd))]; h2=s2[s2>=rise]
            if len(h2): hits.append(pd.Timestamp(h2.index[0].year,h2.index[0].month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=pdy-1))
        if len(hits)>=need:
            hits.sort(); out_.append((max(pp,hits[need-1]),dd))
    return out_
U3=UR.dropna().rolling(3).mean().dropna()
def ur_rolled_over(k=2,tol=0.0):
    """the unemployment rate has stopped rising: its three-month mean is no higher than k months earlier. The rate
    peaks at or after the trough, never before, so this cannot fire early - and it is published on the employment
    report, five days into the following month."""
    out_={}
    for m,v in U3.items():
        p_=m-pd.DateOffset(months=k)
        if p_ in U3.index and v<=U3[p_]-tol: out_[m]=rel.get(m,pd.Timestamp(m.year,m.month,1)+pd.DateOffset(months=1)+pd.Timedelta(days=4))
    return out_
def confirm_ur(calls,k,tol,fwd=6):
    RO=ur_rolled_over(k,tol); out_=[]
    for pp,dd in calls:
        hits=[p_ for m,p_ in RO.items() if dd<=m<=dd+pd.DateOffset(months=fwd)]
        if hits: out_.append((max(pp,min(hits)),dd))
    return out_
def settle_band(nm,band=0.02,n=3,L=12,stable=3,back=48,tolm=1):
    """the settled date need only be stable within tolm months, not identical: a rule that insists on the same month
    three times waits for the level to stop moving at all"""
    lv,d=CH[nm]; lv=lv.rolling(n).mean().dropna()
    out_=[]; run=[]; seen=set()
    for m in pd.date_range('1948-01-01','2026-08-01',freq='MS'):
        avail=lv[lv.index<=m-pd.DateOffset(months=1)]
        if len(avail)<8: continue
        dt=ch_date(avail,m-pd.DateOffset(months=back),avail.index[-1],band,1,L)
        if dt is None: run=[]; continue
        mm=pd.Timestamp(dt.year,dt.month,1); run.append(mm)
        if len(run)>=stable:
            w=run[-stable:]
            if (max(w)-min(w)).days<=tolm*31:
                cand=max(w)
                if cand not in seen: seen.add(cand); out_.append((_pub(nm,avail.index[-1]),cand))
    return out_
P("TWO NEW GUARDS. First, the unemployment rate having stopped rising - it peaks at or after the trough, never before,")
P("so it cannot confirm early, and it arrives on the employment report. Second, a settled date that need only hold")
P("within a month rather than be identical three times running.")
S2=confirm_close(TLG['S'],0.15,2.0,need=2)
base={**TLH,'S':S2}
R1=confirm_close2(bristow_closer('the insured rate, inverted',0.02,3,12,6,back=48),0.10,2.0,2)
trow('  shipped closers, S rebuilt',base)
trow('  + the insured rate settling at six months',{**base,'R':R1})
import itertools
P("")
P("housing starts settling, guarded by the unemployment rate rolling over")
for stable,k in itertools.product([4,6,8],[1,2,3]):
    c=first_to_settle(stable=stable,back=48,chans=['housing starts'])
    cc=confirm_ur(c,k,0.0)
    if not cc: continue
    trow(f'  starts {stable}m, UR rolled over over {k}m ({len(cc)} calls)',{**base,'R':R1,'T':cc})
P("")
P("the insured rate settling with a one-month tolerance on the settled date")
for stable,tolm in itertools.product([3,4,6],[0,1,2]):
    c=settle_band('insured rate',stable=stable,tolm=tolm)
    cc=confirm_close2(c,0.10,2.0,2)
    if not cc: continue
    trow(f'  insured rate {stable}m stable, date tolerance {tolm}m ({len(cc)} calls)',{**base,'R':cc})
out.close()
