...
THE OBJECTS, STANDARDIZED - the second attempt.  In raw units (log points for housing, points for the rate) the
line does NOT transfer: leave-one-economy-out picks 7.5 log points without Canada, 40.0 without Japan and 17.5
without Korea, and each held-out economy then shows either an exposure of twenty to thirty-six per cent or a
coverage of two in fourteen.  The reason is plain once seen - Canadian dwelling PERMITS, Japanese construction
WORK STARTED and Korean permits are differently constructed and differently volatile, so a fall of fifteen log
points does not mean the same thing in each.  The fix is to measure each object in units of ITS OWN quiet-period
standard deviation, which is scale-free by construction and is how this program compares objects everywhere else:

    HOUSING   (the k-month mean below its trailing `back`-month maximum) / its own quiet standard deviation
    LABOUR    (the unemployment rate above its trailing `back`-month minimum) / its own quiet standard deviation
...
Output intl_pair_calibration_sd.log.
"""
import sys, warnings; warnings.filterwarnings('ignore')
import numpy as np, pandas as pd
D = '/home/claude/lab/intlpair'
log = open('/home/claude/lab/weekly/intl_pair_calibration_sd.log', 'w')
...
def _quiet_mask(idx, chron):
    q = pd.Series(True, index=idx)
    for p, t in chron: q[(idx >= M(p) - pd.DateOffset(months=9)) & (idx <= M(t) + pd.DateOffset(months=18))] = False
    return q
def _std(o, chron):
    q = _quiet_mask(o.index, chron); sd = float(o[q].std())
    return o / sd if sd > 0 else o
def pct_fall(s, k, back, chron=None):
    m = (np.log(s.clip(lower=1e-9)) * 100).rolling(k).mean()
    o = (m.shift(1).rolling(back).max() - m).dropna()
    return _std(o, chron) if chron is not None else o
def lvl_rise(s, k, back, chron=None):
    m = s.rolling(k).mean(); o = (m - m.shift(1).rolling(back).min()).dropna()
    return _std(o, chron) if chron is not None else o
...
def evaluate(k, back, lh, lu, econs=None, drop=None):
    ...
    for nm in (econs or BOOK):
        B = BOOK[nm]
        H = pct_fall(B['h'], k, back, B['chron']); U = lvl_rise(B['u'], k, back, B['chron'])
...
GRID = [(k, back, float(lh), float(lu))
        ...
        for lh in np.arange(0.5, 4.05, 0.125) for lu in np.arange(0.5, 4.05, 0.125)]
P(f'{len(GRID)} candidate settings on the grid (k, back, housing and rate lines 0.5-4.0 in each object own quiet standard deviations).')
...
g0, s0 = best()
c, t, e = evaluate(*g0)
P(f'  best setting k={g0[0]} back={g0[1]} housing {g0[2]:.3f} sd AND rate {g0[3]:.3f} sd: '
...
for nm in BOOK:
    ...
    P(f'  leave out {nm:8s}: picks k={g[0]} back={g[1]} housing {g[2]:.3f} sd AND rate {g[3]:.3f} sd; '
...
for g, n in cnt.most_common(6):
    P(f'    k={g[0]} back={g[1]} housing {g[2]:.3f} sd AND rate {g[3]:.3f} sd  chosen in {n} of {len(picks2)} folds')
...
P(f'\nTHE FOREIGN SAMPLE\'S CHOICE, to be handed to the American test unchanged: '
  ...
  f'housing {cnt.most_common(1)[0][0][2]:.3f} sd AND rate {cnt.most_common(1)[0][0][3]:.3f} sd')
log.close()
