...
log = open('/home/claude/lab/weekly/second_pairs.log', 'w')
...
for a, c in itertools.combinations(names, 2):
    ...
    for la in GRID[a]:
        for lc in GRID[c]:
            ...
            if w73 >= B1973 and w07 >= B2007: continue
...
H = pd.DataFrame(hits)
H.to_csv('/home/claude/lab/weekly/second_pairs.csv', index=False)
P(f'\n{len(H)} pair settings pass all four gates AND move a wall.')
if len(H):
