...
PL,TL=AC.legs(safe=True, with_S=True, with_U=True); turns=B.american_chronology(PL,TL,sahm=AC.sahm_rt(),second=[dict(name='vacancy(2,6)',gap=AC.vacancy_gap_rt(2,6),line=0.36,pub_day=30)])   # the default from version 43 (leg U added: the insured unemployment rate): the rate OR the vacancy rate's fast form, first prints; leg S (Paper 1's end) gated by the claims field's arming
