...
def housing_rate_pair(hline=30.0, uline=0.20, hk=2, hback=12, uk=1, uback=12):
    """VERSION 44's third alternative in the second condition: HOUSING COLLAPSING AND UNEMPLOYMENT RISING.

    Returns one series whose value reaches 1.0 exactly in the months where BOTH hold:

        housing starts, the `hk`-month mean at least `hline` LOG POINTS below its trailing `hback`-month maximum
        AND the unemployment rate, the `uk`-month reading at least `uline` POINTS above its trailing `uback`-month
        minimum

    Both are read as FIRST PRINTS - ALFRED's housing-starts vintages begin 21 July 1960 and its unemployment-rate
    vintages 15 March 1960, so 1967, 1973, 1980, 1981, 1990, 2001, 2007 and 2020 are every one of them the figure
    the release actually printed.  Housing starts begin in January 1959, so the pair CANNOT be read at the 1951 and
    1952 claims calls at all: it is admissible only as a THIRD ALTERNATIVE beside the two objects that reach 1948,
    never as a replacement for them.  Public the 18th of the month after (housing starts are the later release).

    Why a pair, and why this pair.  A single fast object cannot separate the February 1967 credit crunch from a
    recession: every one tried in memos 8y, 8aa, 8ab and 8ac was refused on 1967.  A pair fires only where both
    readings stand at their lines in the SAME month, and 1966-67 is exactly the episode in which housing collapsed
    while the unemployment rate went from 3.8 to 3.9 per cent.  The clause is the committee's own distinction.

    Its window exposure over the quiet months since 1949 is ZERO: it never stands at its line outside a downturn,
    so it adds nothing whatever to the route's false-alarm hazard (`v44_hazard.log`).
    """
    ...
    def _rt(sid):
        ...
        fp = _al.first_prints(sid)
        return pd.concat([cur[cur.index < fp.index.min()], fp]).sort_index()
    hs = _rt('HOUST'); ur = _rt('UNRATE')
    hm = (np.log(hs) * 100).rolling(hk).mean()
    H = (hm.shift(1).rolling(hback).max() - hm).dropna()
    um = ur.rolling(uk).mean()
    U = (um - um.shift(1).rolling(uback).min()).dropna()
    return pd.concat([H / float(hline), U / float(uline)], axis=1).dropna().min(axis=1)


def second_v44():
    """The version-44 second condition as the tool takes it: the vacancy rate's fast form and the housing/rate pair
    (the unemployment rate at Sahm's 0.5 is passed separately as `sahm=`).  Use with horizon_months=6."""
    return [dict(name='vacancy(2,6)', gap=vacancy_gap_rt(2, 6), line=0.36, pub_day=30),
            dict(name='housing+rate', gap=housing_rate_pair(), line=1.0, pub_day=18)]


HORIZON_V44 = 6   # the confirmation window is symmetric: back_months before the claims call, the same after
...
if __name__ == '__main__':
    ...
    # ---- VERSION 44: the symmetric confirmation window and the housing/rate pair -------------------------
    _s44 = second_v44()
    score(B.american_chronology(PLU, TLG, sahm=g, second=_s44, horizon_months=HORIZON_V44),
          "ROUTE B'' + leg S + leg U + THE HOUSING/RATE PAIR, symmetric window - THE VERSION-44 DEFAULT")
    print("\nthe chronology under VERSION 44, every turn:")
    for t in B.american_chronology(PLU, TLG, sahm=g, second=_s44, horizon_months=HORIZON_V44):
        if t['kind'] == 'peak':
            print(f"  peak   called {t['published']:%Y-%m-%d} by {t['leg']}  dated {t['date']:%Y-%m}  "
                  f"(claims call {t['claims_published']:%Y-%m-%d}; condition {t['condition']})")
        else:
            print(f"  trough called {t['published']:%Y-%m-%d} by {t['leg']}  dated {t['date']:%Y-%m}")
