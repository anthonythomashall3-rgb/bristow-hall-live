...
_iu=pd.read_csv('/home/claude/archive/data/fred/IURSA.csv'); _iu.columns=['d','v']; _iu=_iu.set_index(pd.to_datetime(_iu['d']))['v'].astype(float)
_g=(_iu-_iu.rolling(52,min_periods=26).min()).dropna()
print(f"U  the insured unemployment rate above its 52-week minimum (leg U, line 0.50): latest {_g.index.max():%Y-%m-%d} = {_g.iloc[-1]:+.2f} points; maximum since 2022 {_g['2022':].max():.2f} in {_g['2022':].idxmax():%Y-%m-%d}")
_z = AC.housing_rate_pair()
_hs = pd.read_csv('/home/claude/archive/data/fred/HOUST.csv'); _hs.columns=['d','v']; _hs=_hs.set_index(pd.to_datetime(_hs['d']))['v'].astype(float).dropna()
_ur = pd.read_csv('/home/claude/archive/data/fred/UNRATE.csv'); _ur.columns=['d','v']; _ur=_ur.set_index(pd.to_datetime(_ur['d']))['v'].astype(float).dropna()
_hm=(np.log(_hs)*100).rolling(2).mean(); _H=(_hm.shift(1).rolling(12).max()-_hm).dropna()
_um=_ur.rolling(1).mean(); _U=(_um-_um.shift(1).rolling(12).min()).dropna()
print(f"P  the housing/rate pair (version 44's third alternative; both must hold in the same month):")
print(f"     housing starts below their 12-month maximum (2-month mean, line 30.0 log points): latest {_H.index.max():%Y-%m} = {_H.iloc[-1]:.1f}")
print(f"     the unemployment rate above its 12-month minimum (line 0.20 points):              latest {_U.index.max():%Y-%m} = {_U.iloc[-1]:.2f}")
print(f"     the pair (the smaller of the two in units of its own line; 1.00 is the line):     latest {_z.index.max():%Y-%m} = {_z.iloc[-1]:.2f}  -> {'AT ITS LINE' if _z.iloc[-1] >= 1 else 'below'}")
PL,TL=AC.legs(safe=True, with_S=True, with_U=True); turns=B.american_chronology(PL,TL,sahm=AC.sahm_rt(),second=AC.second_v44(),horizon_months=AC.HORIZON_V44)   # the default from version 44: the confirmation window symmetric about the claims call (six months either side), and the second condition = the rate at Sahm's 0.5 OR the vacancy rate's fast form OR the housing/rate pair, all on first prints; leg U on the peak side, leg S (Paper 1's end) gated by the claims field's arming
