...
DEFAULT (version 44 = version 43 plus the SYMMETRIC CONFIRMATION WINDOW and the HOUSING/RATE PAIR).  The
    confirmation window is now six months either side of the claims call (horizon_months=6, matching back_months),
    where it ran up to eighteen months forward before; and the second condition gains a third alternative, housing
    starts thirty log points below their trailing twelve-month maximum AND the unemployment rate 0.20 points above
    its trailing twelve-month minimum, in the same month, on first prints, published the 18th.  Peaks 12/12,
    median 28 days, worst 90, four inside the peak month, mean date error 1.08; the November 1973 and December 2007
    walls both removed.  See american_chronology.housing_rate_pair() and HORIZON_V44.  (Version 43 = version 39
    plus leg U, the insured unemployment rate: the Department's own weekly
    rate - claims paid over covered employment - one week, 0.50 points above its 52-week minimum, published the
    Thursday after the week, dated to that week's month; form and line chosen leave-one-recession-out, every fold
    the same; seven calls in fifty-five years, every one inside a recession window, so no quiet-period call and no
    measurable hazard added under the conjunction.  It takes the route's median onset lag from 40 to 30 days, the
    worst from 142 to 126, the within-a-month count from 5 to 6, and the mean date error from 1.17 to 1.08 months.)

    (version 39 = version 38's onsets, with leg S at the ends): the peak call is the claims objects AND one of two labor-market
