...
sys.path.insert(0, '/home/claude/lab/weekly'); import american_chronology as _AC
...
chk('the memo section 2 opens with version 44 and the header is dated September 4, 2026',
    '**Evidence memo, version 44. Hall & Bristow recession program. September 4, 2026.**' in memo and '**Version 44 removes BOTH remaining walls' in memo and '**Version 43 adopts LEG U' in memo and "**Version 42 puts a NUMBER on Rule 20" in memo and '**Version 41 (4 September 2026, small hours)' in memo and "**Version 40 carries out the three sweeps" in memo and "**Version 39 answers Anthony's question of the night" in memo and "**Version 38 continues the speed hunt under Anthony's clearance" in memo, True)
...
chk('the memo carries section 8l and the route docstring is at version 44 (the symmetric window and the pair)', '## 8l. The fourth pass' in memo and 'DEFAULT (version 44 = version 43 plus the SYMMETRIC CONFIRMATION WINDOW' in B.AMERICAN_ROUTE, True)
...
chk("the 1946-1983 panel, triple-checked (a neighbour check AND the release's own sum identity AND magnitude): "
    '6,261 state-weeks, 54 names, 353 weeks, July 1958 to August 1981',
    len(_pan) == 6261 and _pan.state_name.nunique() == 54 and _pan.week_ic.nunique() == 353
    and f'{_pan.week_ic.min():%Y-%m-%d}' == '1958-07-05' and f'{_pan.week_ic.max():%Y-%m-%d}' == '1981-08-22', True)
chk("the sum identity is enforced: every kept row has iu_all = iu_state + iu_ucfe + iu_ucx within two per cent",
    bool((((_pan.iu_state + _pan.iu_ucfe + _pan.iu_ucx) - _pan.iu_all).abs()
          <= (2.0).__class__(2.0) + 0.02 * _pan.iu_all.abs()).all()), True)
chk('the memo carries 8v and 8w', '## 8v.' in memo and '## 8w.' in memo, True)

print('=== 24. version 43: leg U, and the second-condition hunt (4 September 2026)')
r24a = run('iur_loo.py', cwd=f'{LAB}/weekly')
chk('leg U chosen leave-one-recession-out: every one of the twelve folds picks one week at 0.50 points',
    'every fold the same: True {(1, 0.5)}' in r24a, True)
chk("leg U's own quiet-period calls in fifty-five years: zero", "the leg's own quiet-period calls 0" in r24a, True)
r24b = run('american_chronology.py', cwd=f'{LAB}/weekly')
chk('the version-43 chronology: median 30 d, worst 126, within a month after 7, six exact dates, mae 1.08; twelve peaks and no other but 2023',
    'THE VERSION-43 DEFAULT' in r24b and
    'peaks: 12/12 committee peaks called; other onset calls 1; lag median 30 d, worst 126, inside the month 2, within a month after 7; dates exact 6, within one 8, mae 1.08' in r24b, True)
chk('version 39 for comparison (leg S gated, no leg U): median 40 d, worst 142, within a month after 6, mae 1.17',
    'peaks: 12/12 committee peaks called; other onset calls 1; lag median 40 d, worst 142, inside the month 2, within a month after 6; dates exact 6, within one 8, mae 1.17' in r24b, True)
_ac = open(f'{LAB}/weekly/american_chronology.py').read()
chk('the tool carries leg U and the version-43 default', 'def leg_U(' in _ac and 'with_U' in _ac and 'version 43' in B.AMERICAN_ROUTE, True)
r24c = open(f'{LAB}/weekly/second_hunt2.log').read()
chk('the second-condition hunt: housing starts move both walls (1973 +41, 2007 +18) and are refused - the combined exposure goes 7.16 to 11.78 per cent, a 1.65x multiplier',
    'the shipped OR (Sahm, vacancy) 7.16%' in r24c and 'the OR with housing starts    11.78%' in r24c
    and "the multiplier on the route's hazard: 1.65x" in r24c, True)
chk('the February 1967 margin on the tool\'s own window: +9.30 points, 0.97 standard deviations, and one month earlier the window carries 56.70',
    'margin  +9.30 (+0.97 sd)' in r24c and '56.70 in 1966-10' in r24c, True)
chk('the knife edge: of 96 settings inside the gate, 20 open no outside episode and exactly 2 move both walls',
    'of 96 settings inside the gate, 20 open no episode outside the thirteen; of those, 2 move BOTH walls.' in r24c, True)
r24d = open(f'{LAB}/weekly/second_hunt3.log').read()
chk("the rate in gap form: folds not unanimous and the February 1967 margin exactly zero",
    'every fold the same: False' in r24d and 'February 1967   window 1966-11-1968-04: max  0.600 in 1967-10, margin +0.000 (+0.00 sd)' in r24d, True)
r24e = open(f'{LAB}/weekly/claims_frontier2.log').read()
chk('the claims frontier worked to its end: 22 of 78 weekly claims objects make no quiet-period call, and none of them moves the route',
    '22 of 78 make no quiet-period call in their whole history.' in r24e, True)
chk('the memo carries 8x, 8y and 8z and STANDING-RULES carries version 43',
    '## 8x.' in memo and '## 8y.' in memo and '## 8z.' in memo and 'Version 43 (4 September 2026)' in open('/home/claude/proj/STANDING-RULES.md').read(), True)

print('=== 25. the monthly tier of the second-condition hunt (4 September 2026)')
r25 = open(f'{LAB}/data/second_screen_monthly.log').read()
chk('the monthly tier: 1,328 FRED series beginning by January 1951 and still published, screened analytically',
    'THE MONTHLY TIER: 1328 FRED series beginning on or before January 1951 and still published' in r25, True)
_ss = pd.read_csv(f'{LAB}/data/second_screen_monthly.csv')
_ss['gain'] = (_ss.b1973 - _ss.w1973).clip(lower=0) + (_ss.b2007 - _ss.w2007).clip(lower=0)
chk('the full sweep of all 1,328: 12,422 settings on 643 series pass the exposure and OR gates, 239 move a wall, and construction employment is among them',
    len(_ss) == 12422 and _ss.id.nunique() == 643 and int((_ss.gain > 0).sum()) == 239 and 'USCONS' in set(_ss[_ss.gain > 0].id), True)
r25b = open(f'{LAB}/weekly/uscons_final.log').read()
chk("the gate is the shipped condition's own margin: Sahm +0.46 sd and the vacancy fast form +0.26 sd, both at February 1967",
    'tightest: Sahm +0.46 sd, vacancy +0.26 sd' in r25b, True)
chk('on first prints construction employment loses the 2007 gain: the February 1967 window maximum is 298,000 and the folds split',
    '298.00 in 1968-06' in r25b and 'every fold the same: False' in r25b, True)
chk('the memo carries 8aa and STANDING-RULES carries the monthly tier',
    '## 8aa.' in memo and 'AND THE MONTHLY TIER IS SWEPT TOO' in open('/home/claude/proj/STANDING-RULES.md').read(), True)

print('=== 26. the conjunctive second condition: found, measured, not adopted (4 September 2026)')
r26 = open(f'{LAB}/weekly/second_pairs_all.log').read()
chk('the pair sweep: 24,311 settings pass all four gates and 1,579 move a wall',
    '24311 pair settings pass all four gates' in r26 and 'of them, 1579 move at least one wall' in r26, True)
_pp = pd.read_csv(f'{LAB}/weekly/second_pairs_all.csv')
_pp['A'] = _pp.a.str.split().str[0]; _pp['C'] = _pp.c.str.split().str[0]
_both = (_pp.w1973 < 120) & (_pp.w2007 < 126)
chk('719 settings move BOTH walls, and the effect is one family: housing starts with the unemployment rate (373 of 625) and with the unemployment level (301 of 631)',
    int(_both.sum()) == 719
    and int(_both[(_pp.A == 'HOUST') & (_pp.C == 'UNRATE')].sum()) == 373
    and int(_both[(_pp.A == 'HOUST') & (_pp.C == 'UNEMPLOY')].sum()) == 301, True)
chk('at its best the pair takes 1973 to +41 days and 2007 to -3, at zero added hazard (the OR unchanged at 6.72 per cent)',
    int(_pp.w1973.min()) == 41 and int(_pp.w2007.min()) == -3 and float(_pp.comb.max()) <= 6.73, True)
r26b = open(f'{LAB}/weekly/second_pairs_loo.log').read()
chk('and it is NOT adopted: the leave-one-out folds do not agree on the line',
    'every fold the same: False' in r26b, True)
chk('the memo carries 8ab and STANDING-RULES carries the conjunctive design and the out-of-sample identification',
    '## 8ab.' in memo and 'THE MECHANISM IS ESTABLISHED; THE LINE IS NOT' in open('/home/claude/proj/STANDING-RULES.md').read(), True)

print('=== 27. the out-of-sample identification of the pair, attempted and failed (4 September 2026)')
r27 = open(f'{LAB}/weekly/intl_pair_calibration.log').read()
chk('the foreign calibration sample: 27 contractions in Canada, Japan and Korea, none American',
    'the foreign calibration sample: 27 contractions in three economies, none American' in r27, True)
chk('in raw units the line does not transfer: leave-one-economy-out gives three different housing lines',
    'every fold the same: False' in r27, True)
r27b = open(f'{LAB}/weekly/intl_pair_calibration_sd.log').read()
chk('standardized to each object\'s own quiet standard deviation the foreign coverage rises to 26 of 27, and the folds still disagree',
    'coverage 26/27 (96 per cent)' in r27b and 'every fold the same: False' in r27b, True)
r27c = open(f'{LAB}/weekly/intl_pair_apply.log').read()
chk("the foreign line applied to America unchanged FAILS: exposure 16.92 per cent, the OR 22.78, February 1967 at -0.05 sd, and a false episode on 18 July 1968",
    'window exposure 16.92 per cent' in r27c and 'the OR of the second condition 22.78 per cent' in r27c
    and 'margin -0.045 (-0.05 sd)' in r27c and '1968-07-18' in r27c, True)
chk('and it is fast while it does it - 1973 to +49 days and 2007 to -3 - which is the whole lesson',
    '1973 49 (was 120), 2007 -3 (was 126)' in r27c, True)
chk('the memo carries 8ac and STANDING-RULES records that the walls are a sample-size problem',
    '## 8ac.' in memo and 'THE TWO WALLS ARE A SAMPLE-SIZE PROBLEM' in open('/home/claude/proj/STANDING-RULES.md').read(), True)

print('=== 28. version 44: the symmetric confirmation window and the housing/rate pair (4 September 2026)')
r28 = open(f'{LAB}/weekly/window_cap.log').read()
chk("the twelve's own crossings arrive between five months before and four months after the claims call; the 1967 window ran sixteen",
    'on the twelve: crossings from -5 to +4 months from the claims call' in r28, True)
chk('the cap costs nothing: at every cap from 4 to 36 months the route on the twelve is bit-identical',
    r28.count('     12  [\'2023-08-28\']                 30    126         4  1.08') >= 8, True)
r28b = open(f'{LAB}/weekly/v44_hazard.log').read()
chk("memo 8v's exposure is corrected: on the route's own eighteen-month forward reach version 43 stands at 27.77 per cent, and version 44 at 8.89",
    'forward reach 18 months    27.77%' in r28b and 'forward reach 6 months     8.89%' in r28b
    and 'the pair alone, forward reach 6 months                   0.00%' in r28b, True)
chk("the hazard: version 43 one false episode in 94 years, version 44 one in 292 - divided by 3.12",
    'one false episode every 94 years' in r28b and 'one false episode every 292 years' in r28b
    and 'divides the hazard by 3.12' in r28b, True)
r28c = open(f'{LAB}/weekly/capped_pair_consensus.log').read()
chk('the consensus selection: four settings accepted by all twelve folds, and every one gives the same twelve calls',
    '4 settings accepted by ALL TWELVE folds' in r28c and 'every accepted setting gives the SAME twelve calls: True' in r28c, True)
r28d = open(f'{LAB}/weekly/american_chronology.log').read()
chk('the version-44 chronology: median 28 days, worst 90, four inside the peak month, mae 1.08; both walls removed',
    'THE VERSION-44 DEFAULT' in r28d and
    'peaks: 12/12 committee peaks called; other onset calls 1; lag median 28 d, worst 90, inside the month 4, within a month after 8; dates exact 6, within one 8, mae 1.08' in r28d, True)
chk('November 1973 called 10 January 1974 and December 2007 called 28 December 2007, both dated to the committee\'s month',
    '1974-01-10   U    1974-01   1973-11        41   +2' in r28d and '2007-12-28   C    2007-12   2007-12        -3   +0' in r28d
    and '1980-01-18   U    1980-01   1980-01       -13   +0' in r28d, True)
chk('the tool and the harness carry version 44', 'DEFAULT (version 44' in B.AMERICAN_ROUTE
    and 'def housing_rate_pair(' in open(f'{LAB}/weekly/american_chronology.py').read()
    and 'HORIZON_V44' in open(f'{LAB}/weekly/american_now.py').read(), True)
chk('the memo carries 8ad and withdraws the 8v figure, and STANDING-RULES carries version 44',
    '## 8ad.' in memo and 'is therefore **withdrawn**' in memo
    and 'Version 44 (4 September 2026)' in open('/home/claude/proj/STANDING-RULES.md').read(), True)

import numpy as np, pandas as pd
print('=== 29. what version 44 does not buy: the claims frontier re-closed, the lookback held at six (4 September 2026)')
r29 = open(f'{LAB}/weekly/claims_frontier3.log').read()
chk('under version 44 ten of the twelve are claims-bound, so the claims frontier was re-priced; only three objects gain and two wreck the dates',
    '3 candidates make the route faster without adding a call anywhere' in r29
    and 'mae 1.75' in r29 and 'mae 1.83' in r29, True)
chk('the one clean claims gain is two days at December 1969 and nothing else',
    any('initial claims, 2-week mean 50 log points' in _l and 'mae 1.08' in _l and "gains {'1969-12': 2}" in _l
        for _l in r29.split('\n')), True)
_bk = {}
for back in (3, 6, 7):
    _t = B.american_chronology(*_AC.legs(safe=True, with_S=True, with_U=True), sahm=_AC.sahm_rt(),
                               second=_AC.second_v44(), horizon_months=_AC.HORIZON_V44, back_months=back)
    _pk = [x for x in _t if x['kind'] == 'peak']
    _l = []
    for x in _pk:
        c = [i for i, (p, q) in enumerate(zip(_AC.PK, _AC.TR)) if p - pd.DateOffset(months=6) <= x['date'] <= q]
        if c: _l.append((x['published'] - _AC.month_end(_AC.PK[c[0]])).days)
    _bk[back] = (len(_l), float(np.median(_l)), sum(v <= 0 for v in _l))
chk('the lookback: six months gives median 28 and four inside the peak month; seven gives 18 and five, and is refused because it raises the exposure',
    _bk[6] == (12, 28.0, 4) and _bk[7] == (12, 18.0, 5) and _bk[3] == (12, 30.0, 3), True)
chk('the memo carries 8ae with both refusals', '## 8ae.' in memo and 'Ten days of median speed is not worth' in memo, True)
