#!/usr/bin/env python3
"""Build the Missed Recession paper as a .docx matching the v5.0 layout exactly.
...
def run(par, text, size=12, bold=False, italic=False, sup=False):
...
def para(doc, align=WD_ALIGN_PARAGRAPH.JUSTIFY, first_indent=None, left=None,
         right=None, before=0, after=8, leading=17.0, hanging=None, keep_next=False):
    p = doc.add_paragraph()
...
def emit_inline(p, text, size=12, base_italic=False):
...
FOOTNOTE_TEXT = 'The authors thank Gioele Abbati and students in FBE 437 Summer Session for helpful comments.'
...
def build(md_path, out_path):
    ...
    while i < n:
        ...
        if line.strip().strip('*').startswith('Anthony Hall'):
            ...
            run(p, 'Anthony Hall and Duke K. Bristow', size=12)
        ...
        if m:
            ...
            r = p.add_run()
            r.add_picture(os.path.join(FIGDIR, m.group(1)), width=Inches(float(m.group(2))))
            i += 1; continue
        ...
        if line.strip().startswith('- '):
            ...
            emit_inline(p, line.strip()[2:], size=12)
            i += 1; continue
