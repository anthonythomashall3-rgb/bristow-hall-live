...
for c in ALL:
    for x in run_country_concept(c,**K):
        if not (x['hp'] and x['ht']): miss.append((c,x['peak_off'],x['tr_off'],x['nch'],CONCEPT[c]))
GRID=[(bt,bp,cap,md) for bt in (0.01,0.02,0.03,0.05) for bp in (0.01,0.02,0.03,0.05)
      for cap in (9,12,18,24) for md in (3.0,5.0,8.0)]
print(f'{"chronology":24s} {"episode":12s} {"nch":>3s} {"routed":10s} {"level":>7s} {"growth":>7s} {"diff":>7s}')
for c,pk_off,tr_off,nch,cpt in miss:
    ...
    n=len(GRID)
    print(f'{c:24s} {str(pk_off):12s} {nch:3d} {cpt:10s} {cnt["level"]:4d}/{n} {cnt["growth"]:4d}/{n} {cnt["diffusion"]:4d}/{n}')
