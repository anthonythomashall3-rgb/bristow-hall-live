...
def dev(x, L=12, n=3):
    ...
    if e is not None and e[0] is x: return e[1]
    m=ma(x,n); mx=m.shift(1).rolling(L).max()
    r=(mx-m)/mx*100.0; _DEV[k]=(x,r); return r
...
def date_diffusion_panel(chs, w0, w1, line=45.0, run=1, band=0.55, n_di=5, n_band=3, L=12, run_p=4):
...
def run_country_concept(country, lead=12, tail=12, concept=None, **kw):
    ...
    for _e in cfg['chrono']:
        ...
        if g is not None and g.index.min()<=w0 and g.index.max()>=trm:
            tr=ch_trough(g,w0,w1,kw.get('band_t',0.03),1,4,abstain=False)
            pk=ch_peak(g,w0,tr if tr is not None else w1,kw.get('band_p',0.01),1,abstain=False)
            if pk is not None and tr is not None:
                ...
                rows.append(dict(country=country,peak_off=pk_off,tr_off=tr_off,nch=len(use),
                                 ...
                                 verdict='quarterly GDP')); continue
        if cpt=='diffusion':
