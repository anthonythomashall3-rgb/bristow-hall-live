...
def ch_trough(level, w0, w1, band=0.02, n=3, L=12, tadj=False, win=120, minp=60, abstain=None,
    ...
    return min(d_peak,p) if p is not None else d_peak
...
def run_country_concept(country, lead=12, tail=12, concept=None, **kw):
    ...
    for _e in cfg['chrono']:
        ...
        if ds is not None:
            _nm,g,_n,_L=ds
            tr=ch_trough(g,w0,w1,kw.get('band_t',0.12),_n,_L,abstain=False)
            pk=ch_peak(g,w0,tr if tr is not None else w1,kw.get('band_p',0.01),_n,abstain=False)
