...
def legs(safe=True, with_S=False):
    A = U.leg_A(); C = U.leg_C()
    ...
    import os; Pn = pd.read_csv(f"/home/claude/lab/fh/{os.environ.get('CLAIMS_FIELD','FH')}_state_claims_sa_rt_log.csv", index_col=0, parse_dates=True)   # 5 Sep 2026: CLAIMS_FIELD=HYB
    ...
    return dict(A=A, B=Bw, C=C, M=M), TL
