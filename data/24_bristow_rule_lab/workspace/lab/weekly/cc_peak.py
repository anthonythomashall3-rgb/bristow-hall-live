"""The peak detector on continued-claims breadth alone.
Continued claims are the layoff-persistence measure; initial claims are the layoff-flow
measure.  They are not the same information and they do not turn together."""
...
def peaks(Bd,thb,reb,r,mp,line=50.,pub=7):
    ...
    for i in range(r,len(b)):
        ...
        if on:
            ...
            out.append((idx[i]+pd.Timedelta(days=pub),mo(idx[j]))); state='called'; off=0
    return out
def sc(calls,cut='1988-01'):
    ...
    return got,fa
rows=[]
for sm,L,gap,thb,reb,r,mp in itertools.product((13,26),(52,78,104,156),(15.,20.,25.,30.,40.),
                                               (40.,45.,50.,55.,60.),(20.,25.,30.,40.),(2,4),(26,52,78)):
    g,fa=sc(peaks(breadth(sm,L,gap),thb,reb,r,mp))
    rows.append((len(g),-len(fa),(sm,L,gap,thb,reb,r,mp),dict(g),fa))
rows.sort(key=lambda x:(-x[0],x[1]))
for b in rows[:8]:
    print(f'{b[0]}/5 false {-b[1]}  cfg {b[2]}')
    print(f'   hits {b[3]}')
