...
def score(tag):
    ...
    print(f"{tag:34s} peaks {g(ep,0):2d}/{g(ep,1):2d}/{g(ep,3):2d} of {len(ep)} bias {sum(ep)/len(ep):+.2f} | troughs {g(et,0):2d}/{g(et,1):2d}/{g(et,3):2d} of {len(et)} bias {sum(et)/len(et):+.2f}  errs P {ep} T {et}")
for conv,ref in itertools.product(('tool','esri'),(0,1,2)):
    bench.PHASE_CONV=conv; bench.PHASE_REFINE=ref
    score(f'conv={conv} refine={ref}')
