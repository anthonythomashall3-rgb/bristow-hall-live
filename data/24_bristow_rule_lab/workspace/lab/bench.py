...
PANELS={
 ...
 # Employment: the OECD's PME-based series ends in November 2015 (IBGE closed the PME); the
 # successor survey's employed persons (PNAD Continua, table 6318, rolling quarter, from
 # March 2012; keyless SIDRA API) is adjusted with the program's real-time month-of-year
 # routine and chained onto the PME series at the 2012-03..2015-11 overlap (2 September 2026;
 # lab/nat/bra).  Tested before adoption: no end moves except the 2016-Q4 trough, from one
 # quarter early to the quarter.
 'Brazil': dict(chrono=BR_ALL, freq='M', src='CODACE',
    ch=kei('BRA', rs=True, skip={'employment'},
           extra=[ex(f'{LAB}/cand/BRA_IBCBR.csv',name='monthly GDP proxy'),
                  ex(f'{LAB}/nat/bra/BRA_employment_chained.csv',name='employment')])),
 ...
 # The Banco de Espana's Boletin Estadistico carries four monthly PRODUCTION VOLUMES
 # reaching back before the OECD's Spanish panel does - cement production from January
 # 1955, steel production from January 1968, and gasoline and diesel consumption from
 # January 1969 - through a free keyless CSV service.  They are the same class of object
 # as the channels already voting, and they take the 1974 contraction from two channels
 # to six and the 1978 contraction from three to seven.  Chapters 23 and 15 of the
 # bulletin, www.bde.es/webbe/es/estadisticas/compartido/datos/csv/.
 'Spain': dict(chrono=ES_Q, freq='Q', src='SBCDC (AEE)',
    ch=[(nm,(f'{SPLICED}/ESP_ip.csv' if nm=='industrial production' else p_),k)
       ...
       +P(ex('/home/claude/lab/esp/ESP_cement_production.csv',name='cement production'),
          ex('/home/claude/lab/esp/ESP_steel_production.csv',name='steel production'),
          ex('/home/claude/lab/esp/ESP_gasoline_consumption.csv',name='gasoline consumption'),
          ex('/home/claude/lab/esp/ESP_diesel_consumption.csv',name='diesel consumption'))),
...
def _plateau_pick(on, where):
    ...
    sample.  'mid' takes the center of the flat region instead, which is unbiased by
    ...
    the center with a twelve-percent band eight times in nine.
    """
...
def ch_trough(level, w0, w1, band=0.02, n=3, L=12, tadj=False, win=120, minp=60, abstain=None,
    ...
    # (3 September 2026) a channel rising from a low that is the window's first month, its
    # deviation statistic at its maximum there, carries the previous trough, not this one
    if _ab and TROUGH_EDGE_ABSTAIN=='joint' and i==m.index[0] and d_peak==d.index[0]: return None
    if _ab and TROUGH_EDGE_ABSTAIN=='dpeak' and d_peak==d.index[0]: return None
    if _ab and TROUGH_EDGE_ABSTAIN=='both' and i==m.index[0]: return None
    hi=float(m[:i].max()) if len(m[:i]) else float(m.max())
    amp=max(hi-lo,1e-9)
    on=m[m<=lo+band*amp]
    p=_plateau_pick(on,where)
    t=max(d_peak,p) if p is not None else d_peak
    if refine and REFINE_T is not None and n>1 and where=='mid':
        t=_refine(level,t,REFINE_T,'min',REFINE_SMOOTH_T,w0,w1)
    if FLAT_BETA and where=='mid' and n>1:
        # flat-floor experiment: when the smoothed level stays within FLAT_BETA of its minimum
        # for FLAT_MIN months or more after that minimum, the trough is the last such month
        after=m[i:]; wide=after[after<=lo*(1.0+FLAT_BETA)]
        # the run must be contiguous from the minimum
        run_end=i
        for d_ in after.index:
            if d_ in wide.index: run_end=d_
            else: break
        if md_(run_end,i)>=FLAT_MIN: t=run_end
    return t
FLAT_BETA=0.0     # experiment (2 Sep): 0 = off; e.g. 0.02 = two per cent of the minimum
FLAT_MIN=12
def md_(x,y): return (x.year-y.year)*12+(x.month-y.month)

# ---------------------------------------------------------------- the refinement clause
# A trailing n-month mean lags the extremum it finds by about (n-1)/2 months.  After the
# smoothed clauses have placed a turning point, the date moves to the extremum of the
# (lightly smoothed) level inside a window of a few months around it - the month without
# the noise.  Level clause only; a diffusion index keeps its last-month reading.
REFINE_T=(3,1); REFINE_P=(1,0); REFINE_SMOOTH_T=1; REFINE_SMOOTH_P=2   # shipped 2026-09-02
import os as _os
if _os.environ.get('BR_REFINE'):      # held-out runs: BR_REFINE="T=1,1;P=1,0;rsT=1;rsP=2;scope=all"
    _spec=_os.environ['BR_REFINE']
    if _spec=='1': _spec='T=1,1;P=1,0;rsT=1;rsP=2;scope=all'
    for _kv in _spec.split(';'):
        _k,_v=_kv.split('=')
        if _k=='T': REFINE_T=None if _v=='none' else tuple(int(x) for x in _v.split(','))
        elif _k=='P': REFINE_P=None if _v=='none' else tuple(int(x) for x in _v.split(','))
        elif _k=='rsT': REFINE_SMOOTH_T=int(_v)
        elif _k=='rsP': REFINE_SMOOTH_P=int(_v)
        elif _k=='scope': pass
TROUGH_EDGE_ABSTAIN=_os.environ.get('BR_TROUGH_EDGE') or 'joint'   # 3 September 2026: mirrors bristow_rule_v3
REFINE_BOUNDED=(_os.environ.get('BR_REFINE_BOUNDED') or 'yes')=='yes'  # the refinement stays inside [w0, w1]
def _refine(level, date, window, kind, rs, w0=None, w1=None):
    back,fwd=window
    x=level if rs<=1 else ma(level,rs)
...
def ch_peak(level, w0, w1, band=0.02, n=3, tadj=False, win=120, minp=60, abstain=None,
    ...
    if refine and REFINE_P is not None and n>1 and where=='mid':
        p=_refine(level,p,REFINE_P,'max',REFINE_SMOOTH_P,w0,w1)
    return p
...
PHASE_CONV='tool'      # 'tool': the turning-point month opens the new phase (peak month = contraction);
                       # 'esri': the turning-point month closes the old one (peak month = last expansion month,
                       #         trough month = last contraction month), which is how ESRI's tables read
PHASE_REFINE=0         # months: move each Bry-Boschan turning point (found on the smoothed series) to the
                       # unsmoothed series' extremum within +/- this many months - Bry and Boschan's final step
DI_BB='tool'           # 'tool': bry_boschan on a 3-month centred mean (shipped); 'std': the full Bry-Boschan
                       # sequence (12-month MA, Spencer curve, MCD, final on the unsmoothed series)
def phase_series(level, n=3):
    ...
    if DI_BB=='std':
        sys.path.insert(0,'/home/claude/lab/acq/esri/kenkyukai')
        from bb_standard import bry_boschan_std
        tp,_=bry_boschan_std(level)
    else:
    ...
    if PHASE_REFINE:
        k=PHASE_REFINE; tp2=[]
        for d,kind in tp:
            seg=level[d-pd.DateOffset(months=k):d+pd.DateOffset(months=k)].dropna()
            if len(seg): d=seg.idxmax() if kind=='P' else seg.idxmin()
            tp2.append((d,kind))
        tp=tp2
    ...
    if PHASE_CONV=='esri':
        for (d,k) in tp:
            nxt=d+pd.DateOffset(months=1)
            if nxt in ph.index: ph[nxt] = 0.0 if k=='P' else 1.0
    else:
        for (d,k) in tp: ph[d] = 0.0 if k=='P' else 1.0
    ph=ph.ffill()
    first=tp[0]
    ph[:first[0]] = 1.0 if first[1]=='P' else 0.0
    if PHASE_CONV=='esri': ph[first[0]] = 1.0 if first[1]=='P' else 0.0
    return ph
...
# ----------------------------------------------------------------- always answers
TRIM=24     # months: a channel date further than this from the median is dropped

def med_fallback(dates_abstain, dates_all, trim=None):
    ...
    Then trim: a channel whose date is more than two years from the panel's median is
    not describing the same turning point - it has locked onto a different episode
    inside the search window - so it is dropped and the median retaken.  Two years is
    a full cycle by Bry and Boschan's own minimum-cycle censoring.  The trim needs at
    least four voting channels to apply, and never leaves fewer than two.  It costs
    no hits and cuts the mean trough error from 1.65 months to 1.62.  Chosen out of
    sample: leaving each chronology out in turn, the other eight pick two years eight
    times in nine, and the result is flat from eighteen months to twenty-four."""
    t = TRIM if trim is None else trim
    ds=[x for x in dates_abstain if x is not None]
    if not ds: ds=[x for x in dates_all if x is not None]
    d=med(ds)
    if d is None or t is None or len(ds)<4: return d
    c=d.year*12+(d.month-1)
    keep=[x for x in ds if abs((x.year*12+(x.month-1))-c)<=t]
    return med(keep) if len(keep)>=2 else d
...
def date_any(country, chs, w0, w1, band_t=0.03, band_p=0.02, n=3, L=12, peak_cap=12,
    ...
    # --- level form
    ch_t=[(nm,s) for nm,s in chs if nm not in SKIP_TROUGH] or chs
    ...
    if REFINE_T is not None:
        # the peak-search window ends at the UNREFINED trough, so the refinement of
        # the trough never moves the window inside which the peak is looked for
        _u=med_fallback([ch_trough(s,w0,w1,band_t,n,L,abstain=True,refine=False) for nm,s in ch_t],
                        [ch_trough(s,w0,w1,band_t,n,L,abstain=False,refine=False) for nm,s in ch_t])
    ...
    if ref is not None and len(ma(ref,n)[w0:w1].dropna())>=6:
        ...
        _gu=ch_trough(ref,w0,w1,0.0,n,L,abstain=False,refine=False)
        gend=_gu if _gu is not None else w1
        g_pk=ch_peak(ref,w0,gend,band_p,n,abstain=False,refine=False)
    else:
        cy=cyc_hp(q,lam,n)
...
# important" but alongside labor-market and industrial-production indicators.  Both
...
# The C.D. Howe Institute's Business Cycle Council, Cross and Bergevin, "Turning Points:
# Business Cycles in Canada since 1926", C.D. Howe Institute Commentary 366, p. 4:
# "for recent decades, we use quarterly GDP and employment data to identify probable
# instances of recession, with the exact monthly data then refined with either monthly
# GDP post-1961 or industrial production prior to 1961, along with monthly employment."
# The two ends are not symmetric in that document.  The peak is set by the output series
# alone - 1960: "Industrial production peaked in March 1960, which points to this month as
# the peak for this cycle"; 1990: "Monthly GDP dropped sharply in April 1990, followed
# shortly by employment, which points to a cyclical peak in March 1990."  The trough is
# set by output and employment together - 1961: "The cyclical trough in both output and
# jobs was in March 1961"; 1992: "Monthly GDP and employment started to level off in
# April 1992 ... This points to a trough of April 1992."  Canada is therefore routed to
# monthly GDP from 1961 and industrial production before it for the peak, and to that
# series together with monthly employment for the trough, wherever employment reaches
# back far enough (Statistics Canada's seasonally adjusted monthly employment starts in
# 1953; the panel here starts in 1955).
_KOR_CO=None
def kor_coincident():
    """Statistics Korea's own coincident composite index, cyclical component.

    KOSIS table DT_1C8015 is behind a registration wall.  The same series is
    republished by the Korean government's national indicator portal, e-NARA / 지표누리,
    as indicator 1057 'Composite economic indexes', statistical table 105701, monthly
    from January 1970, sourced there to Statistics Korea's Monthly Industrial Activity
    Trends.  It is fetched with no key from
    index.go.kr/unity/index/IndexTblGraphAjax.do."""
    global _KOR_CO
    if _KOR_CO is None:
        pth='/home/claude/lab/kor/KOR_coincident_cyclical.csv'
        _KOR_CO=load(pth) if os.path.exists(pth) else False
    return _KOR_CO if _KOR_CO is not False else None

def dating_series(country, w0, trm):
    """The series a committee documents as the ones it dates, when it names them and
    they cover the episode.  Returns None when the committee reads a panel.

    The third element of the tuple may be a single series, a list of series used at
    both ends, or a dict {'peak': [...], 'trough': [...]} when the document uses
    different evidence at the two ends."""
    if country=='Korea':
        co=kor_coincident()
        if co is not None and co.index.min()<=w0 and co.index.max()>=trm:
            return ("Statistics Korea's own coincident composite index, cyclical component",
                    co, 3, 12)
    g=qgdp(country)
    ...
    if country=='Canada':
        ...
        def _ok(s): return s is not None and s.index.min()<=w0 and s.index.max()>=trm
        core=None; nm=None
        if _ok(gdp): core,nm=gdp,'monthly GDP'
        elif _ok(ip): core,nm=ip,'industrial production'
        if core is not None:
            spec={'peak':[core],'trough':[core]}
            tag=nm
            if _ok(emp):
                spec['trough'].append(emp); tag=nm+' with employment'
            di=ca_diffusion()
            if di is not None and di.index.min()<=w0 and di.index.max()>=trm:
                spec['trough_diffusion']=[di]; tag=tag+' and the industry diffusion index'
            return (tag, spec, 3, 12)
    return None

# The same document names a third piece of evidence and uses it only at the trough:
# "we use a diffusion index for GDP, which Statistics Canada formerly calculated, that
# shows the percentage of industries that expand output in a particular month", and for
# 1992: "Monthly GDP and employment started to level off in April 1992, one month after
# the diffusion index struggled to reach the 50 percent barrier."  Statistics Canada no
# longer publishes it, so it is rebuilt here from Statistics Canada's own monthly GDP by
# industry - table 36-10-0378 (the fourteen 1980 SIC divisions, 1961 to 1997) joined to
# table 36-10-0434 (the twenty two-digit NAICS sectors, 1997 on) - as the share of those
# industries whose seasonally adjusted constant-price output rose that month.  Putting it
# in the peak as well was tested and is worse: Canada's peaks go from 11 of 12 to 10.
_CADI=[None,False]
def ca_diffusion():
    if not _CADI[1]:
        p='/home/claude/lab/statcan/CA_gdp_diffusion.csv'
        _CADI[0]=load(p) if os.path.exists(p) else None
        _CADI[1]=True
    return _CADI[0]

def _ds_lists(spec):
    """Normalise the third element of a dating_series tuple to
    (peak_list, trough_list, trough_diffusion_list)."""
    if isinstance(spec, dict):
        return list(spec.get('peak',[])), list(spec.get('trough',[])), list(spec.get('trough_diffusion',[]))
    if isinstance(spec, list): return list(spec), list(spec), []
    return [spec], [spec], []

# A committee that dates quarters is dated at quarterly frequency.  The AFSE's committee
# already is, because it names a quarterly series; the CEPR-EABCN and Spanish committees
# read panels of monthly indicators but publish quarterly dates, and dating their panels
# month by month and then rounding to a quarter mixes two frequencies.  The panel is
# therefore averaged to quarters and the same two clauses are applied with no further
# smoothing and a four-quarter lookback - the same n and L the French route already uses.
_QAGG={}
def to_quarterly(nm, s):
    k=(nm,id(s)); e=_QAGG.get(k)
    if e is None or e[0] is not s:
        e=(s, s.resample('QS').mean().dropna()); _QAGG[k]=e
    return e[1]

def panel_spread(use, w0, w1, band_p=0.01, band_t=0.12, n=3, L=12):
    """First-to-last spread of the panel's own channel dates, in months, at each end.

    This is computed from the panel whatever route produced the answer, so that the
    confidence tier means the same thing on every episode.  Only the channels that vote
    are counted, so the spread describes the evidence that was actually used.
    """
    dp=[ch_peak(s,w0,w1,band_p,n,abstain=True) for nm,s in use]
    if not any(d is not None for d in dp):
        dp=[ch_peak(s,w0,w1,band_p,n,abstain=False) for nm,s in use]
    dt=[ch_trough(s,w0,w1,band_t,n,L,abstain=True) for nm,s in use]
    if not any(d is not None for d in dt):
        dt=[ch_trough(s,w0,w1,band_t,n,L,abstain=False) for nm,s in use]
    return spread_m(dp), spread_m(dt)
...
def run_country_concept(country, lead=12, tail=12, concept=None, **kw):
    ...
    for _e in cfg['chrono']:
        ...
        if ds is not None:
            _nm,g,_n,_L=ds
            gp,gt,gd=_ds_lists(g)
            td=[ch_trough(x,w0,w1,kw.get('band_t',0.12),_n,_L,abstain=False) for x in gt]
            # a diffusion index is read with the diffusion clause wherever it appears:
            # the band that finds the floor of a diffusion index is wider than the one
            # that finds the floor of a level, and the month taken is the LAST inside it
            td+=[ch_trough(x,w0,w1,0.30,3,12,abstain=False,where='last') for x in gd]
            tr=med([x for x in td if x is not None])
            pk=med([ch_peak(x,w0,tr if tr is not None else w1,kw.get('band_p',0.01),_n,abstain=False) for x in gp])
            if pk is not None and tr is not None:
                a,ep=hit(pk,pk_off,freq); b,et=hit(tr,tr_off,freq)
                _sp,_st=panel_spread(use,w0,w1,kw.get('band_p',0.01),kw.get('band_t',0.12))
                rows.append(dict(country=country,peak_off=pk_off,tr_off=tr_off,nch=len(use),
                                 pk=pk,tr=tr,ep=ep,et=et,hp=a,ht=b,depth=None,sp=_sp,st=_st,
                                 verdict=_nm)); continue
        if freq=='Q':
            uq=[(nm,to_quarterly(nm,s)) for nm,s in use]
            kwq={k:v for k,v in kw.items() if k not in ('n','L')}
            base=date_any(country,uq,w0,w1,n=1,L=4,**kwq)
            pk,tr,v=base['peak'],base['trough'],base['verdict']
            a,ep=hit(pk,pk_off,freq); b,et=hit(tr,tr_off,freq)
            _sp,_st=panel_spread(uq,w0,w1,kw.get('band_p',0.01),kw.get('band_t',0.12),1,4)
            rows.append(dict(country=country,peak_off=pk_off,tr_off=tr_off,nch=len(uq),
                             pk=pk,tr=tr,ep=ep,et=et,hp=a,ht=b,depth=base.get('deepest'),
                             sp=_sp,st=_st,verdict=v)); continue
        if cpt=='diffusion':
        ...
        _sp,_st=panel_spread(use,w0,w1,kw.get('band_p',0.01),kw.get('band_t',0.12))
        rows.append(dict(country=country,peak_off=pk_off,tr_off=tr_off,nch=len(use),
                         pk=pk,tr=tr,ep=ep,et=et,hp=a,ht=b,depth=None,sp=_sp,st=_st,verdict=v))
    return rows
...
# ------------------------------------------------------------------ manifest
MANIFEST='/home/claude/lab/bench_manifest.json'

def manifest_check(path=MANIFEST, strict=True, verbose=True):
    """Check the live panels against the recorded channel manifest.

    ex(), estat(), esri(), imf() and kei() all drop a channel whose file is not on
    disk.  That is what lets one module carry economies with different data, but it
    also means a deleted or truncated file changes the benchmark score without
    changing a line of code.  The manifest records every file each panel resolved
    when the shipped record was measured - its SHA-256, its row count and its span -
    so that a change of that kind is loud instead of silent.

    Returns a list of complaints, empty when the panels are exactly as recorded.
    With strict=True a non-empty list is raised rather than returned.
    """
    import json, hashlib
    man=json.load(open(path))
    bad=[]
    for country, want in man['channels'].items():
        if country not in PANELS:
            bad.append(f'{country}: in the manifest, not in PANELS'); continue
        have={}
        for item in PANELS[country]['ch']:
            if item is None: continue
            nm,p,kind=item; have[p]=(nm,kind)
        for w in want:
            p=w['path']
            if p not in have:
                bad.append(f'{country}: {w["name"]} ({os.path.basename(p)}) is no longer in the panel')
                continue
            if not os.path.exists(p):
                bad.append(f'{country}: {os.path.basename(p)} is missing from disk'); continue
            h=hashlib.sha256(open(p,'rb').read()).hexdigest()
            if h!=w['sha256']:
                s=load(p)
                bad.append(f'{country}: {os.path.basename(p)} has changed '
                           f'({w["rows"]} rows {w["first"]}..{w["last"]} recorded, '
                           f'{len(s)} rows {s.index.min().date()}..{s.index.max().date()} now)')
        extra=set(have)-{w['path'] for w in want}
        for p in sorted(extra):
            bad.append(f'{country}: {have[p][0]} ({os.path.basename(p)}) is new since the manifest')
    n=man['total_channels']
    live=sum(1 for c in man['channels'] if c in PANELS
             for item in PANELS[c]['ch']
             if item is not None and os.path.exists(item[1]))
    if verbose:
        print(f'manifest: {n} channels recorded, {live} on disk now, {len(bad)} complaints')
        for b in bad: print('   ', b)
    if bad and strict:
        raise RuntimeError('the benchmark panels no longer match the manifest:\n  '
                           +'\n  '.join(bad))
    return bad
