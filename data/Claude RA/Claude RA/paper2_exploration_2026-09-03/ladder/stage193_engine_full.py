"""Stage 193: the same engine abroad.  Identical code to the American runs of stage 190 -- the
...
LEVELS_US={"UNRATE","U6RATE","EMRATIO","CIVPART","UEMPMEAN","UEMPMED","TCU","MCUMFN","MSACSR",
 "ISRATIO","AISRSA","NAPM","NAPMNOI","IURSA","UMCSENT","USSLIND"}
for p in glob.glob(FE+"usmod/*.csv"):
    sid=os.path.basename(p)[:-4]; s_=rd(p)
    if len(s_)>2000: s_=s_.resample("MS").mean()
    add("USA","us "+sid,s_,"level" if sid in LEVELS_US else "activity")
for d in glob.glob(FE+"imf_wide/*"):
    iso=I2TO3.get(os.path.basename(d))
    if iso is None: continue
    for p in glob.glob(d+"/*.csv"):
        nm=os.path.basename(p)[:-4]
        add(iso,"ifs "+nm,rd(p),"level" if nm.startswith("LUR") else "activity")
...
for iso,(nc,n,d,f) in sorted(NC.items(),key=lambda kv:-kv[1][0])[:14]:
    print("%-5s %-7d %-9s %d"%(iso,nc,"%d/%d"%(d,n),f))
print("\nDETECTION AGAINST CHANNEL COUNT")
print("%-18s %-10s %-12s %s"%("channels","countries","episodes","detected"))
import numpy as _np
bands=[(0,15),(15,30),(30,60),(60,90),(90,10**6)]
for a,b in bands:
    iso_=[k for k,v in NC.items() if a<=v[0]<b]
    if not iso_: continue
    ep=sum(NC[k][1] for k in iso_); de=sum(NC[k][2] for k in iso_)
    print("%-18s %-10d %-12d %d (%.0f%%)"%("%d to %d"%(a,b if b<10**6 else 999),len(iso_),ep,de,100*de/max(ep,1)))
