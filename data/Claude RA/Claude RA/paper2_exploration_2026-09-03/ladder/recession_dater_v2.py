"""Recession dater v2 (3 Sep 2026, round 5) — v1 plus one weekly ratio channel. Replays 1968-2026 and prints every call.
...
TRIGGER: any channel on and 'fresh' (was off on some day in the prior 120 days):
  ...
  (3) insured unemployment rate, 4-week avg, >= 0.30 pp above its 52-week min  [weekly, known +12 days]   <- new in v2
  (4) nonfarm payrolls, first print, 1-month change <= -0.1%              [ALFRED PAYEMS vintages]
  (5) Michigan consumer sentiment down >= 10 points in one month          [monthly, known at month end; from 1978]
  (6) housing starts, first print, 3-month change <= -20% on two consecutive releases [ALFRED HOUST vintages]
...
chs={"Sahm>=0.35":SAHM[0.35], "claims 8wk>=30%":IC["ic8_30_k1"], "IUR 4wk gap>=0.30pp":IUR4, "payrolls -0.1%":PAY, "sentiment -10":UM["um_d1_10"], "housing starts -20% x2":HOU}
