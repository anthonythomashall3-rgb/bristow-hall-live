"""Stage 185: the international test rebuilt on Eurostat and the OECD.  The old test was limited
...
def build(iso,delta=0.25,floor=0.30,post=9):
    ...
    if "unemployment" in ch:
        u=ch["unemployment"].reindex(idx).interpolate(limit_area="inside")
        S["unemployment"]=u.rolling(3).mean()-u.rolling(12).min()
