...
want = [(r['id'], r['freq'], r['title']) for r in csv.DictReader(open(os.path.join(CAT, 'fred_highfreq_new2.csv')))]
...
with open(os.path.join(OUT, 'MANIFEST_highfreq2.csv'), 'w', newline='') as f:
    w = csv.writer(f); w.writerow(['series', 'freq', 'first', 'last', 'title']); w.writerows(ok)
with open(os.path.join(OUT, 'FAILED_highfreq2.csv'), 'w', newline='') as f:
