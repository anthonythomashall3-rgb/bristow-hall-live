...
D=pd.concat([pd.read_csv(f,parse_dates=['week']).assign(pass1=('tolerant2' not in f)) for f in sorted(glob.glob('iu_tolerant_*.csv'))+sorted(glob.glob('iu_tolerant_NONE_*.csv'))],ignore_index=True)
...
own1=D[D['pass']].groupby(['state','week']).iu.median(); own2=D[~D['pass']].groupby(['state','week']).iu.median()
own=own1.combine_first(own2).rename('own')
imp=D.dropna(subset=['chg']).copy(); imp['prev']=imp.iu-imp.chg; imp['pweek']=imp.week-pd.Timedelta(days=7)
...
if 'chg_yr' in D.columns and D.chg_yr.notna().any():
    imy=D.dropna(subset=['chg_yr']).copy(); imy['prev']=imy.iu-imy.chg_yr; imy['pweek']=imy.week-pd.Timedelta(days=364)
...
R=pd.DataFrame(rows,columns=['state','week','iu','source'])
# plausibility: within [0.1x, 10x] of the state's rolling median of confirmed values
R=R.sort_values(['state','week'])
def plaus(g):
    ref=g[g.source.isin(['both','repaired'])].set_index('week').iu
    if len(ref)<5: return g
    med=ref.rolling(26,min_periods=5,center=True).median().reindex(g.week,method='nearest').values
    bad=(g.iu.values>10*med)|(g.iu.values<0.1*med)
    g=g.copy(); g.loc[bad,'source']='implausible'; g.loc[bad,'iu']=np.nan; return g
R=R.groupby('state',group_keys=False).apply(plaus)
R.to_csv('iu_weekly_state_1945_1983_long.csv',index=False)
W=R.dropna(subset=["iu"]).pivot_table(index='week',columns='state',values='iu')
W.to_csv('iu_weekly_state_1945_1983_wide.csv')
